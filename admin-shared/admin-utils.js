...
function _el(tag, attrs, children) {
  ...
  if (attrs) {
    ...
    if (attrs.dataset) {
      for (var k in attrs.dataset) node.dataset[k] = attrs.dataset[k];
    }
  }
  if (children) {
...
function confirmAdmin(message, onConfirm) {
  ...
  var msg = _el('div', {
    style: 'font-size:14px;color:var(--t1);line-height:1.7;margin-bottom:20px;white-space:pre-line',
  });
...
function ld(k, fb) {
  try {
    var d = localStorage.getItem(SK + k);
...
function sv(k, d) {
  localStorage.setItem(SK + k, JSON.stringify(d));
}
function gid() {
  ...
  sty: ld('sty', [
    {
      id: 's1',
      name: '충격 이슈형',
      desc: '빠른 전개, 강한 훅, 커뮤니티 말투',
      prompt:
        '- 첫 문장은 반드시 충격적인 한 줄로 시작 (예: "이거 실화냐", "방금 터졌습니다")\n- 문장은 5~10자로 극단적으로 짧게 끊기\n- "ㄷㄷ", "레전드", "실화냐", "역대급" 같은 커뮤니티 표현 적극 사용\n- 말투: ~했다고 합니다, ~인 거죠, ~라고 하네요\n- 감탄사 많이: 와, 헐, 대박, 미쳤다\n- 중간에 "근데 여기서 반전" 같은 전환 장치 넣기\n- 마지막에 "이거 어떻게 생각하세요? 댓글로 알려주세요" 같은 참여 유도',
      on: true,
    },
    {
      id: 's2',
      name: '정보 요약형',
      desc: '팩트 중심, 깔끔한 정리, 뉴스 톤',
      prompt:
        '- 첫 문장은 핵심 팩트 한 줄 요약으로 시작 (예: "OO가 OO했습니다")\n- 감정 표현 최소화, 사실 위주로 전달\n- 말투: ~입니다, ~했습니다 (뉴스 앵커 톤)\n- 숫자/날짜/이름 등 구체적 정보 강조\n- 구조: 팩트 → 배경 → 의미 → 전망\n- 불필요한 수식어 제거, 간결하게\n- 마지막에 "더 자세한 내용은 팔로우하고 확인하세요" 같은 CTA',
      on: true,
    },
    {
      id: 's3',
      name: '스토리텔링형',
      desc: '이야기식 전개, 궁금증 유발, 반전 구조',
      prompt:
        '- 첫 문장은 궁금증을 유발하는 질문이나 상황 묘사로 시작 (예: "만약 이런 일이 당신에게 일어났다면?")\n- 이야기하듯 편안한 구어체 사용\n- 말투: ~했거든요, ~인데요, ~잖아요\n- 시간순으로 이야기 전개\n- 중간에 "근데 여기서 문제가 생겼어요" 같은 서스펜스\n- 감정 이입 유도: "이 사람 입장에서 생각해보세요"\n- 마지막에 반전이나 교훈으로 마무리\n- "이 이야기가 도움이 됐다면 팔로우 부탁드려요"',
      on: true,
    },
  ]),
  usr: [],
...
function uc() {
  var el1 = $('styC');
  if (el1) el1.textContent = D.sty.length;
  var el2 = $('usrC');
  if (el2) el2.textContent = D.usr.length;
}

// ── 사이드바 탭 전환 ──
function goTab(t) {
  document.querySelectorAll('.side-item[data-t]').forEach(function (x) {
