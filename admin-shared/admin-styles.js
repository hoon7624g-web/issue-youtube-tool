...
function rS() {
  ...
  headLeft.appendChild(
    _el('div', { className: 'page-desc', textContent: '스크립트 생성에 사용되는 스타일 프리셋' })
  );
  ...
  var thead = _el('thead');
  var hRow = _el('tr');
  ['스타일명', '설명', '프롬프트', '상태'].forEach(function (t) {
    hRow.appendChild(_el('th', { textContent: t }));
  });
  ...
  D.sty.forEach(function (s) {
    ...
    tr.appendChild(
      _el('td', {
        style: 'max-width:200px;font-size:12px;color:var(--t3)',
        textContent:
          (s.prompt || '').substring(0, 50) + (s.prompt && s.prompt.length > 50 ? '...' : ''),
      })
    );
    var tdToggle = _el('td');
    ...
    actDiv.appendChild(
      _el('button', {
        ...
        textContent: '수정',
        dataset: { action: 'edit-style', sid: s.id },
      })
    );
    actDiv.appendChild(
      _el('button', {
        ...
        textContent: '삭제',
        dataset: { action: 'delete-style', sid: s.id },
      })
    );
...
function oSt(id) {
  ...
  header.appendChild(
    _el('div', { className: 'md-t', textContent: editing ? '스타일 수정' : '스타일 추가' })
  );
  header.appendChild(
    _el('button', { className: 'md-x', textContent: '×', dataset: { action: 'close-modal' } })
  );
  ...
  footer.appendChild(
    _el('button', {
      className: 'btn btn-p',
      textContent: '저장',
      dataset: { action: 'save-style', sid: editing ? s.id : '' },
    })
  );
...
function sSt(id) {
  ...
  if (id) {
    var i = D.sty.findIndex(function (s) {
      return s.id === id;
    });
    if (i > -1) {
      o.id = id;
...
// ── 이벤트 위임 액션 등록 ──
ACTIONS['add-style'] = function () {
  oSt(null);
};
ACTIONS['edit-style'] = function (el) {
  oSt(el.dataset.sid);
};
ACTIONS['delete-style'] = function (el) {
  dSt(el.dataset.sid);
};
ACTIONS['save-style'] = function (el) {
  sSt(el.dataset.sid || null);
};
ACTIONS['toggle-style'] = function (el) {
  tSt(el.dataset.sid, el.checked);
};
