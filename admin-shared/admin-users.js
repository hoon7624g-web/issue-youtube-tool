...
function rU() {
  ...
  [
    ...
    sel.appendChild(_el('option', { value: o.v, textContent: o.t }));
  });
  ...
  filterRow.appendChild(
    _el('button', {
      ...
      id: 'bulkApproveBtn',
      style: 'display:none',
      textContent: '✓ 선택 승인 (0명)',
      dataset: { action: 'bulk-approve' },
    })
  );
  ...
  usrList.appendChild(
    _el('div', {
      style: 'padding:40px;text-align:center;color:var(--t3)',
      textContent: '로딩 중...',
    })
  );
...
function loadUsers() {
  adminFetch('/admin/users')
    ...
    .catch(function (e) {
      ...
      el.appendChild(
        _el('div', {
          style: 'padding:40px;text-align:center;color:var(--red)',
          textContent: '회원 목록 로드 실패: ' + e.message,
        })
      );
    });
}
...
function filterUsers() {
  ...
  if (usrFilter !== 'all')
    list = list.filter(function (u) {
      return u.approval_status === usrFilter;
    });
  if (usrSearch) {
    var q = usrSearch.toLowerCase();
    list = list.filter(function (u) {
      return (
        (u.full_name || u.name || '').toLowerCase().indexOf(q) !== -1 ||
...
function renderUserTable(list) {
  ...
  var pending = list.filter(function (u) {
    return u.approval_status === '대기중';
  });
  ...
  if (pending.length) {
    ...
    countRow.appendChild(
      _el('span', {
        style: 'color:var(--yel);font-weight:600',
        textContent: '승인 대기 ' + pending.length + '명',
      })
    );
  }
  ...
  ['이름', '이메일', '연락처', '기수', '승인 상태', '가입일'].forEach(function (t) {
    headRow.appendChild(_el('th', { textContent: t }));
  });
  ...
  list.forEach(function (u) {
    ...
    var tdChk = _el('td');
    tdChk.appendChild(
      _el('input', { type: 'checkbox', dataset: { uid: u.id, onChange: 'toggle-select' } })
    );
    tr.appendChild(tdChk);
    tr.appendChild(_el('td', { textContent: u.full_name || u.name || '-' }));
    tr.appendChild(
      _el('td', { style: 'font-family:var(--mono);font-size:12px', textContent: u.email || '' })
    );
    ...
    tdStatus.appendChild(
      _el('span', {
        className: 'badge ' + (u.approval_status === '승인완료' ? 'badge-grn' : 'badge-acc'),
        textContent: u.approval_status === '승인완료' ? '승인완료' : '대기중',
      })
    );
    ...
    tr.appendChild(
      _el('td', {
        ...
        textContent: u.created_at ? new Date(u.created_at).toLocaleDateString('ko') : '',
      })
    );
    var tdAct = _el('td');
    ...
    if (u.approval_status === '대기중') {
      actDiv.appendChild(
        _el('button', {
          ...
          textContent: '승인',
          dataset: { action: 'approve-user', uid: u.id },
        })
      );
      actDiv.appendChild(
        _el('button', {
          ...
          textContent: '삭제',
          dataset: { action: 'delete-user', uid: u.id },
        })
      );
    } else {
      actDiv.appendChild(
        _el('button', {
          ...
          textContent: '승인 취소',
          dataset: { action: 'reject-user', uid: u.id },
        })
      );
      actDiv.appendChild(
        _el('button', {
...
function bulkApprove() {
  ...
  confirmAdmin(ids.length + '명을 승인하시겠습니까?', function () {
    ...
    ids.forEach(function (uid) {
      adminFetch('/admin/approve', { method: 'POST', body: JSON.stringify({ user_id: uid }) }).then(
        function () {
          done++;
          if (done === ids.length) {
            toast(done + '명 승인 완료');
...
// ── 이벤트 위임 액션 등록 ──
ACTIONS['refresh-users'] = function () {
  rU();
};
ACTIONS['usr-search'] = function (el) {
  usrSearch = el.value;
  filterUsers();
};
ACTIONS['usr-filter'] = function (el) {
  usrFilter = el.value;
  filterUsers();
};
...
ACTIONS['approve-user'] = function (el) {
  approveUser(el.dataset.uid);
};
ACTIONS['reject-user'] = function (el) {
  rejectUser(el.dataset.uid);
};
ACTIONS['delete-user'] = function () {
  deleteUser();
};
ACTIONS['bulk-approve'] = function () {
  bulkApprove();
};
