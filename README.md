...
| 데스크톱 패키징    | 구성 완료 | Windows 인스톨러 빌드 확인               |

## 테스트와 품질 관리

표준 툴링과 프로젝트 자체 회귀 검사를 함께 사용합니다.

- **ESLint**(flat config) — Electron Main(CJS) / 렌더러(ESM) / 빌드 스크립트를 환경별로 린트
- **Prettier** — 코드 포맷 통일 (`npm run format`, `format:check`로 검증 게이트)
- **Vitest** — DOM 비의존 순수 로직(`src/js/pure-utils.mjs`)의 실제 동작 단위 테스트 (라인 커버리지 ~96%, `npm run coverage`)
- **Playwright E2E** — Electron 앱을 실제로 띄워 렌더러가 로그인 화면까지 마운트되는지 확인하는 스모크 (`test/e2e/app.spec.mjs`)
- 버전 정합성 검사 (`scripts/check-version.js`)
- 구문·프로젝트 구조 회귀 검사 (`scripts/structure-test.js`, `scripts/unit-test.js`)
- 스모크 검사 (`scripts/smoke-test.js`)
- Vite 프로덕션 빌드

`npm run verify`로 lint·포맷 검사·단위 테스트·빌드를 한 번에 돌립니다. Electron 앱을 직접 띄우는 E2E 스모크는 디스플레이가 필요해 `npm run build && npm run test:e2e`로 별도 실행합니다.

## 현재 한계

- 외부 API의 정책·응답 형식 변경에 영향을 받을 수 있다.
- 런타임 E2E는 로그인 화면 스모크 수준이며, 전체 흐름 커버리지는 제한적이다.
