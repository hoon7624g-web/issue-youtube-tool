...
function registerElevenLabsIPC(ipcMain, assertTrustedSender, asString, readEncryptedKeys) {
  ...
  ipcMain.handle('call-elevenlabs-tts', async (event, text, voiceId, speed, requestId) => {
    ...
    try {
      ...
      if (res.statusCode === 401)
        return { error: 'ElevenLabs API 키가 유효하지 않습니다.', status: 401 };
      if (res.statusCode >= 400)
        return { error: 'ElevenLabs 오류: ' + res.statusCode, status: res.statusCode };
  ...
  ipcMain.handle('upload-elevenlabs-voice', async (event, payload) => {
    ...
    if (!payload || !payload.bytes || !payload.name)
      return { ok: false, error: '파일 데이터가 없습니다.' };
    try {
      const fileBuffer = Buffer.from(payload.bytes);
      if (fileBuffer.length > 20 * 1024 * 1024)
        return { ok: false, error: '파일이 너무 큽니다 (20MB 이하)' };
      ...
      const fileHeader =
        ...
        (payload.name || 'voice.mp3') +
        '"\r\nContent-Type: ' +
        (payload.type || 'audio/mpeg') +
        '\r\n\r\n';
      const tail = '\r\n--' + boundary + '--\r\n';
      ...
      if (res.statusCode === 401)
        return { ok: false, error: 'ElevenLabs API 키가 유효하지 않습니다.' };
      if (res.statusCode >= 400)
        return { ok: false, error: 'ElevenLabs 업로드 오류: ' + res.statusCode };
