...
function registerTTSIPC(ipcMain, assertTrustedSender, asString, readEncryptedKeys) {
  ...
  ipcMain.handle('call-tts', async (event, text, voiceName, gender, speed, requestId) => {
    ...
    voiceName =
      typeof voiceName === 'string' && voiceName.length < 50 ? voiceName : 'ko-KR-Neural2-B';
    ...
    try {
      ...
      if (res.statusCode === 403)
        return {
          error: 'Google TTS API 키가 유효하지 않거나 API가 활성화되지 않았습니다.',
          status: 403,
        };
      if (res.statusCode >= 400)
        return {
          error: (json.error && json.error.message) || 'TTS API 오류',
