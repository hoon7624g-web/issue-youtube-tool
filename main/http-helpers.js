...
function httpsPost(options, body, timeout, maxResponseBody, signal) {
  ...
  return new Promise((resolve) => {
    ...
    if (signal && signal.aborted)
      return _resolve({ statusCode: 499, body: '', error: 'cancelled', cancelled: true });
    ...
    const req = https.request(options, (res) => {
      ...
      res.on('end', () => {
        if (!tooLarge) _resolve({ statusCode: res.statusCode, body: data });
      });
    });
    const cleanupAbort = _attachAbort(signal, req, () =>
      _resolve({ statusCode: 499, body: '', error: 'cancelled', cancelled: true })
    );
    req.on('error', (e) => {
      if (signal && signal.aborted)
        return _resolve({ statusCode: 499, body: '', error: 'cancelled', cancelled: true });
    ...
    req.setTimeout(timeout, () => {
      req.destroy();
      _resolve({ statusCode: 0, body: '', error: '요청 시간 초과' });
    });
    req.write(body);
    req.end();
  });
...
function httpsPostBuffer(options, body, timeout, maxResponseBody, signal) {
  ...
  return new Promise((resolve) => {
    ...
    const req = https.request(options, (res) => {
      ...
      res.on('end', () => {
        if (!tooLarge) _resolve({ statusCode: res.statusCode, body: Buffer.concat(chunks) });
      });
    });
    const cleanupAbort = _attachAbort(signal, req, () =>
      _resolve({ statusCode: 499, body: Buffer.alloc(0), error: 'cancelled', cancelled: true })
    );
    req.on('error', (e) => {
      if (signal && signal.aborted)
        return _resolve({
      ...
      _resolve({
        statusCode: 0,
        body: Buffer.alloc(0),
        error: _maskKey(e.message) || '네트워크 오류',
      });
    });
    req.setTimeout(timeout, () => {
      req.destroy();
      _resolve({ statusCode: 0, body: Buffer.alloc(0), error: '요청 시간 초과' });
    });
...
function httpsStream(options, body, onChunk, onDone, onError, timeout, maxResponseBody, signal) {
  ...
  if (signal && signal.aborted) {
    _onError({ statusCode: 499, error: 'cancelled', cancelled: true });
    return null;
  }
  ...
  const req = https.request(options, (res) => {
    if (res.statusCode >= 400) {
      ...
      res.on('end', () => {
        if (!tooLarge) _onError({ statusCode: res.statusCode, body: data });
      });
      return;
    }
  ...
  cleanupAbort = _attachAbort(signal, req, () =>
    _onError({ statusCode: 499, error: 'cancelled', cancelled: true })
  );
  req.on('error', (e) => {
    if (signal && signal.aborted)
      return _onError({ statusCode: 499, error: 'cancelled', cancelled: true });
    _onError({ statusCode: 0, error: _maskKey(e.message) });
  });
  req.setTimeout(timeout, () => {
    req.destroy();
    _onError({ statusCode: 0, error: '스트리밍 요청 시간 초과' });
  });
  req.write(body);
  req.end();
  return req;
