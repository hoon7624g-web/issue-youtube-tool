...
function safeError(msg) { return maskKey(typeof msg === 'string' ? msg : String(msg || '')); }

// ★ Fix B: sender 파괴 체크 — 창 닫힘/페이지 이동 시 main process 예외 방지
function safeSend(sender, channel, ...args) {
  if (!sender || sender.isDestroyed()) return false;
  try { sender.send(channel, ...args); return true; }
...
async function callProvider({ hostname, path, headers, body, timeout, errorMap, extractText, label, signal }) {
  const res = await httpsPost({
    hostname, path, method: 'POST',
    headers: { 'Content-Type': 'application/json', ...headers, 'Content-Length': Buffer.byteLength(body) }
  }, body, timeout, undefined, signal);
  ...
  try {
    ...
    if (errorMap && errorMap[res.statusCode]) return { error: errorMap[res.statusCode], status: res.statusCode };
    if (res.statusCode === 429) return { error: E.rate_limit, status: 429 };
    if (res.statusCode >= 400) return { error: safeError((json.error && json.error.message) || label + ' API 오류'), status: res.statusCode };
    const text = extractText(json);
    return { text, status: 200 };
  } catch(e) { return { error: safeError(e.message), status: 500 }; }
}

// ── 텍스트 추출 함수 (프로바이더별) ──
function extractClaude(json) { return json.content ? json.content.map(c => c.text).join('') : ''; }
...
function registerLLMIPC(ipcMain, assertTrustedSender, asString, isValidVideoId, readEncryptedKeys) {
  ...
  ipcMain.handle('call-claude', async (event, prompt, model, maxTokens, requestId) => {
    ...
    try {
      const result = await callProvider({
        ...
        body: JSON.stringify({ model, max_tokens: maxTokens, messages: [{ role: 'user', content: prompt.substring(0, 30000) }] }),
        errorMap: { 401: E.claude_invalid_key },
        extractText: extractClaude, label: 'Claude', signal: controller.signal
      });
  ...
  ipcMain.handle('call-gemini', async (event, prompt, model, maxTokens, requestId) => {
    ...
    try {
      const result = await callProvider({
        hostname: 'generativelanguage.googleapis.com', path: geminiPath(model, 'generateContent', apiKey),
        headers: {},
        body: JSON.stringify({ contents: [{ parts: [{ text: prompt.substring(0, 60000) }] }], generationConfig: { maxOutputTokens: maxTokens } }),
        errorMap: { 400: E.gemini_invalid_key },
        extractText: extractGemini, label: 'Gemini', signal: controller.signal
      });
      return entry.cancelled ? { cancelled: true, status: 499 } : result;
    } finally {
      _finishRequest(requestId, entry);
  ...
  // ── Gemini Video ──
  ipcMain.handle('call-gemini-video', async (event, videoId, prompt, model, maxTokens, requestId) => {
    assertTrustedSender(event);
    ...
    try {
      const result = await callProvider({
        ...
        errorMap: { 400: E.gemini_invalid_key },
        extractText: extractGemini, label: 'Gemini Video', signal: controller.signal
      });
      return entry.cancelled ? { cancelled: true, status: 499 } : result;
    } finally {
      _finishRequest(requestId, entry);
  ...
  ipcMain.handle('call-openai', async (event, prompt, maxTokens, requestId) => {
    ...
    try {
      const result = await callProvider({
        ...
        body: JSON.stringify({ model: MAIN_CONFIG.DEFAULT_OPENAI_MODEL, max_tokens: maxTokens, messages: [{ role: 'user', content: prompt.substring(0, 30000) }] }),
        errorMap: { 401: E.openai_invalid_key },
        extractText: extractOpenAI, label: 'OpenAI', signal: controller.signal
      });
      return entry.cancelled ? { cancelled: true, status: 499 } : result;
    } finally {
      _finishRequest(requestId, entry);
  ...
  ipcMain.handle('call-perplexity', async (event, prompt, maxTokens, requestId) => {
    ...
    try {
      const result = await callProvider({
        ...
        body: JSON.stringify({ model: 'sonar', messages: [{ role: 'user', content: prompt.substring(0, 30000) }], max_tokens: maxTokens }),
        timeout: 120000,
        errorMap: { 401: E.perplexity_invalid_key },
        extractText: extractOpenAI, label: 'Perplexity', signal: controller.signal
      });
      return entry.cancelled ? { cancelled: true, status: 499 } : result;
    } finally {
      _finishRequest(requestId, entry);
...
function registerLLMStreamIPC(ipcMain, assertTrustedSender, asString, readEncryptedKeys, isValidVideoId) {
  ...
  ipcMain.handle('cancel-llm-stream', (event, requestId) => {
    ...
    if (!entry) return { ok: true, cancelled: false };
    entry.cancelled = true;
    try { if (entry.req && !entry.req.destroyed) entry.req.destroy(new Error('cancelled')); } catch (_) {}
  ...
  ipcMain.handle('call-claude-stream', async (event, prompt, model, maxTokens, requestId) => {
    ...
    return new Promise(resolve => {
      ...
      const req = httpsStream({
        ...
        headers: { 'Content-Type': 'application/json', 'x-api-key': keys.claude, 'anthropic-version': '2023-06-01', 'Content-Length': Buffer.byteLength(body) }
      }, body,
      // onChunk
      (payload) => {
        if (entry && entry.cancelled) { try { req.destroy(); } catch(_) {} return; }
        try {
      ...
      () => {
        ...
        if (cancelled) { resolve({ cancelled: true, status: 499 }); return; }
      ...
      (err) => {
        ...
        if (err.body) {
          try { const parsed = JSON.parse(err.body); if (parsed.error && parsed.error.message) msg = parsed.error.message; } catch(_) {}
        }
  ...
  ipcMain.handle('call-gemini-stream', async (event, prompt, model, maxTokens, requestId) => {
    ...
    return new Promise(resolve => {
      ...
      const req = httpsStream({
        ...
        path: geminiPath(model, 'streamGenerateContent', apiKey).replace('?key=', '?alt=sse&key='),
        method: 'POST',
      ...
      (payload) => {
        ...
        try {
          const evt = JSON.parse(payload);
          const text = evt.candidates && evt.candidates[0] && evt.candidates[0].content
            && evt.candidates[0].content.parts && evt.candidates[0].content.parts[0]
  ...
  ipcMain.handle('call-gemini-video-stream', async (event, videoId, prompt, model, maxTokens, requestId) => {
    requestId = requestId || 'default';
    assertTrustedSender(event);
