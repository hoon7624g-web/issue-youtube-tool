...
async function renderThumbnailBatch(params, mainWindow) {
  ...
  try {
    ...
    for (var i = 0; i < variants.length; i++) {
      ...
      try {
        var composition = await _selectComposition({ serveUrl: bundlePath, id: compositionId, inputProps: inputProps });
        ...
        results.push({
          ...
          style: v.style, accentColor: v.accentColor,
          compositionId: compositionId,
          label: v.label || (v.style + ' · ' + v.accentColor),
        });
      } catch (e) {
        log.error('[Remotion] 변형', i, '렌더링 실패:', e.message);
        results.push({ index: i, ok: false, error: e.message, style: v.style, accentColor: v.accentColor, label: v.label || '' });
      }
    }
...
function registerRemotionIPC(ipcMain, assertTrustedSender, mainWindowGetter) {
  function getMW() { return typeof mainWindowGetter === 'function' ? mainWindowGetter() : mainWindowGetter; }
  function sendProgress(stage, pct, msg) {
    try { var mw = getMW(); if (mw && !mw.isDestroyed()) mw.webContents.send('remotion-progress', { stage: stage, pct: pct, msg: msg }); } catch(e) {}
  }
  ...
  ipcMain.handle('remotion-render-thumbnail', async function(event, params) {
    assertTrustedSender(event);
    try {
      var savePath = await renderThumbnail({ title: params.title, backgroundUrl: params.backgroundUrl, accentColor: params.accentColor, channelName: params.channelName, style: params.style, onProgress: sendProgress }, getMW());
      return { ok: true, path: savePath };
    } catch (e) {
      if (e.message === 'CANCELED') return { ok: false, canceled: true };
      return { ok: false, error: e.message };
