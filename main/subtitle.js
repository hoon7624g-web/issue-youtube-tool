...
async function fastExtract(videoId) {
  ...
  if (!jsonText || typeof jsonText === 'object') {
    const reason =
      jsonText && jsonText.truncated
        ? 'JSON이 스캔 상한(' + MAX_PLAYER_RESPONSE_SCAN / 1024 / 1024 + 'MB)을 초과'
        : '구조 불일치';
...
function slowExtract(videoId, hardenChildWindow) {
  return new Promise((resolve) => {
    ...
    const safetyTimeout = setTimeout(
      () => done({ error: '자막 로딩 시간이 초과되었습니다 (30초).' }),
      30000
    );
    ...
    const hidden = new BrowserWindow({
      ...
      webPreferences: {
        ...
        sandbox: true,
        partition: 'subtitle-' + videoId + '-' + Date.now(),
      },
    });
    ...
    hidden.webContents.session.webRequest.onCompleted(
      ...
      async (details) => {
        ...
        try {
          ...
          if (!u.hostname.endsWith('youtube.com') && !u.hostname.endsWith('google.com')) return;
        } catch (e) {
          return;
        }
        try {
          const _u = new URL(details.url);
          if (!_u.pathname.includes('/api/timedtext')) return;
        } catch (_) {
          return;
        }
        ...
        try {
          await hidden.webContents.executeJavaScript(
            'window.__captionUrl = ' + JSON.stringify(String(details.url)) + ';'
          );
    ...
    hidden.webContents.on('did-finish-load', () => {
      _timers.push(
        setTimeout(() => {
          if (!resolved) done({ error: 'YouTube 자막 API를 감지하지 못했습니다.' });
        }, 20000)
      );
      _timers.push(
        setTimeout(async () => {
  ...
  ipcMain.handle('get-subtitle', async (event, videoId) => {
    ...
    if (_slowJobs.size >= MAX_CONCURRENT_SLOW) {
      ...
      return {
        videoId,
        text: '',
        error: '자막 추출 대기열이 가득 찼습니다. 잠시 후 다시 시도해주세요.',
      };
    }
...
module.exports = {
  registerSubtitleIPC,
  getFailCacheCleanupInterval: () => _failCacheCleanupInterval,
};
