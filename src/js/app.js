...
import {
  initApiKeys, initSession, patchApi, setupVoiceHandlers
} from '../client-proxy.js';
import { fetchServerConfig } from '../config.js';
import {
  buildSb, syncSb, showP, buildPanels, newProject, doLogout
} from './ui.js';
...
function _toggleOfflineBanner(offline) {
  ...
  if (offline && !banner) {
    ...
    banner.style.cssText = 'position:fixed;top:60px;left:0;right:0;z-index:9999;background:#c92a2a;color:#fff;text-align:center;padding:8px;font-size:13px;font-weight:500';
...
window.addEventListener('offline', () => { _toggleOfflineBanner(true); toast('인터넷 연결이 끊겼습니다', 'err'); });
window.addEventListener('online', () => { _toggleOfflineBanner(false); toast('인터넷이 다시 연결되었습니다'); });
...
((() => {
  ...
  if (saved === 'dark' || ((!saved) && window.matchMedia && window.matchMedia('(prefers-color-scheme:dark)').matches)) {
    ...
    setTimeout(() => { const btn = $('themeToggle'); if (btn) btn.classList.add('on'); }, 0);
  }
}))();
...
function boot() {
  ...
  buildSb();
  buildPanels();
  sOn(K.NAV_STEP, () => { syncSb(); showP(); trackStep(S.nav.step, 'enter'); });
  sOn(K.NAV_MX, syncSb);
  syncSb();
  showP();
}
...
document.addEventListener('keydown', (e) => {
  ...
  if (e.ctrlKey && e.key === 'ArrowRight') { e.preventDefault(); if (S.nav.step < S.nav.mx) sNext(); }
  if (e.ctrlKey && e.key === 'ArrowLeft') { e.preventDefault(); sPrev(); }
  if (e.ctrlKey && e.key === 'Enter') {
    e.preventDefault();
    // 현재 step의 실행 버튼 클릭
    const btn = document.querySelector('.pnl.on .btn.bp.btn-lg:not(:disabled)') || document.querySelector('.pnl.on .btn.bp:not(:disabled)');
    if (btn) btn.click();
  }
});

// safeStorage에서 API 키 + 세션 캐시 로드 후 UI 시작
Promise.all([initApiKeys(), initSession()]).then(() => {
