...
export function mountThumbnailTab(container) {
  ...
  root.appendChild(
    el('p', {
      className: 'sub',
      textContent: 'AI로 문구를 생성하고, 배경을 선택한 뒤 프리뷰를 확인하세요.',
    })
  );
  ...
  const topicInput = el('input', {
    ...
    type: 'text',
    placeholder: '주제 입력 후 AI 문구 생성 (예: AI 부업, 쇼핑 숏폼 수익화)',
    style: 'flex:1',
  });
  topicInput.value = _state.topic;
  topicInput.addEventListener('input', (e) => {
    _state.topic = e.target.value;
  });
  ...
  const titleLabel = el('div', {
    style: 'font-size:12px;color:var(--t3);margin:12px 0 4px',
    textContent: '최종 문구 (AI 선택 또는 직접 입력/수정)',
  });
  ...
  const channelInput = el('input', {
    className: 'thumb-input thumb-input-sm',
    type: 'text',
    placeholder: '채널명 (선택사항)',
    maxLength: 30,
    style: 'margin-top:8px',
  });
  channelInput.addEventListener('input', (e) => {
    _state.channelName = e.target.value;
  });
  ...
  bgArea.appendChild(
    makeBgCard('🔍', 'Pexels 검색', '무료 이미지', () => showPexelsSearch(bgSection))
  );
  bgArea.appendChild(
    makeBgCard('📁', '로컬 업로드', '내 컴퓨터', async () => {
  ...
  bgArea.appendChild(
    makeBgCard('🎨', '없음', '그라디언트', () => {
      _state.backgroundUrl = null;
      _state.backgroundLocalPath = null;
      _state.backgroundPreview = null;
      renderBgPreview(bgSection);
      clearPreviews();
    })
  );
  ...
  const genBtn = el('button', {
    className: 'thumb-actions btn-primary',
    style: 'width:100%;padding:14px;font-size:15px;border-radius:var(--r);text-align:center',
  });
...
async function generateAiTitles(section, btn) {
  ...
  try {
    ...
    if (!result) {
      toast('LLM API 키가 설정되지 않았습니다.', 'err');
      return;
    }
...
function renderBgPreview(bgSection) {
  ...
  const img = el('img');
  img.src = _state.backgroundPreview;
...
async function checkRemotionAvailability(btn) {
  if (!window.electronAPI?.remotionCheck) {
    btn.disabled = true;
    btn.textContent = 'Electron 환경에서만 사용 가능';
    btn.style.opacity = '.4';
    return;
  }
  try {
    ...
    if (!res.available) {
      btn.disabled = true;
      btn.textContent = 'Remotion 미설치 — remotion 폴더에서 npm install 필요';
      btn.style.opacity = '.4';
    }
  } catch (e) {
    btn.disabled = true;
...
function showPexelsSearch(bgSection) {
  ...
  async function doSearch() {
    ...
    if (!q) return;
    if (!window.electronAPI?.pexelsSearch) {
      toast('Pexels API 미설정', 'err');
      return;
    }
    _state.pexelsQuery = q;
    sb.disabled = true;
    sb.textContent = '검색 중...';
    try {
...
async function generatePreviews() {
  ...
  if (!_state.title.trim()) {
    toast('썸네일 문구를 먼저 입력하세요.');
    return;
  }
  ...
  if (window.electronAPI?.onRemotionProgress) {
    unsub = window.electronAPI.onRemotionProgress((d) => {
      loadMsg.textContent = d.msg || '';
      loadPct.textContent = (d.pct || 0) + '%';
    });
  }
  ...
  try {
    ...
    slot.appendChild(
      el('div', {
        style: 'text-align:center;padding:40px;color:var(--t3)',
        textContent: '프리뷰 생성 실패: ' + (e.message || e),
      })
    );
  } finally {
...
function renderPreviewGrid(slot) {
  ...
  slot.appendChild(
    el('div', {
      style: 'font-size:13px;color:var(--t3);margin-bottom:8px',
      textContent: ok.length + '개 — 클릭 선택 후 저장',
    })
  );
  ...
  ok.forEach((preview) => {
    ...
    const dot = el('div', { className: 'color-dot' });
    dot.style.background = preview.accentColor;
