...
function parseAnalysisResult(t) {
  ...
  if (j) {
    ...
    if (typeof summary === 'string' && summary.length > 10) {
      return {
        ...
        structure: Array.isArray(j.structure)
          ? j.structure.map((s) => cleanAI(s))
          : ['전체 내용 참조'],
  ...
  return {
    summary: cleanAI(cleaned.substring(0, 500)) || '분석 완료',
    hooks: ['분석 결과를 확인하세요'],
    structure: ['전체 내용 참조'],
    reasons: ['AI 분석 완료'],
  };
...
async function _genShortsWithRetry(prompt, { signal } = {}) {
  ...
  return (result || []).map((s) => ({
    title: cleanAI(s.title || ''),
    content: cleanAI(s.content || ''),
  }));
}
...
function normalizeFactCheckItem(item) {
  ...
  const st =
    rawStatus === 'safe' || rawStatus === 'warning' || rawStatus === 'uncertain'
      ? rawStatus
      : 'uncertain';
...
function normalizeFootageSceneItem(item) {
  ...
  const rawAltEn = Array.isArray(item.altEn)
    ? item.altEn
    : typeof item.altEn === 'string'
      ? item.altEn
          .split(',')
          .map((x) => x.trim())
          .filter(Boolean)
      : [];
  const altEn = Array.from(
    new Set(
      rawAltEn
        .filter((x) => typeof x === 'string')
        .map((x) => cleanAI(x).trim())
        .filter(Boolean)
    )
  ).slice(0, 8);
  const normalized = {
...
export const Api = {
  ...
  getKw: () => {
    return wait(400).then(() => {
      let stored;
      try {
        stored = JSON.parse(localStorage.getItem('yt_a_kw'));
      } catch (e) {}
      return (stored || M.keywords).filter((k) => {
        return k.on !== false;
      });
    });
  },
  ...
  getIssueLink: async () => {
    ...
    try {
      ...
      console.warn('[IssueLink] 서버 요청 실패:', e.message);
    }
    ...
    if (window.electronAPI && window.electronAPI.isElectron && window.electronAPI.getIssueLink) {
      try {
        ...
        console.warn('[IssueLink] IPC 실패:', e.message);
      }
    }
  ...
  getSubtitle: (videoId) => {
    ...
    if (window.electronAPI && window.electronAPI.isElectron) {
      return window.electronAPI.getSubtitle(videoId).then((d) => {
  ...
  getVids: (kwLabels, duration) => {
    return wait(600).then(() => {
      return M.videos.slice().sort((a, b) => {
  ...
  analyze: (v, transcript, { signal } = {}) => {
    if (!hasKey('llm'))
      return wait(2000).then(() => {
        return Object.assign({}, M.analysis, { _isDemo: true });
      });
    ...
    if (gaiKey && v.id) {
      return callGeminiVideo(v.id, PROMPT.ANALYZE_VIDEO(v), { signal })
        .then((raw) =>
          finalizeAnalysisMeta(parseAnalysisResult(raw), {
            method: 'video',
            requestedMethod: 'video',
          })
        )
        .catch((e) => {
          return Api._analyzeFallback(v, transcript, { signal }).then((result) =>
            finalizeAnalysisMeta(result, {
    ...
    return Api._analyzeFallback(v, transcript, { signal }).then((result) =>
      finalizeAnalysisMeta(result, { method: fallbackMethod })
    );
  },
  ...
  analyzeStream: async (v, transcript, onChunk, { signal } = {}) => {
    ...
    if (gaiKey && v.id) {
      try {
        ...
        return finalizeAnalysisMeta(parseAnalysisResult(rawText), {
          method: 'video',
          requestedMethod: 'video',
        });
      } catch (e) {
  ...
  _analyzeFallback: async (v, transcript, { signal } = {}) => {
    ...
    const strictParse = (t) => {
      const j = extractJSON(t);
      if (j && typeof j.summary === 'string' && j.summary.length > 10 && Array.isArray(j.hooks))
        return j;
      return null;
    };
  ...
  genScript: (ana, sty, styPrompt) => {
    if (!hasKey('llm'))
      return wait(2500).then(() => {
        return Object.assign({}, M.script, { _isDemo: true });
      });
  ...
  genScriptDual: async (ana, sty, styPrompt) => {
    if (!hasKey('llm')) {
      await wait(2500);
      return {
        longform: Object.assign({}, M.script, { _isDemo: true }),
        shorts: [],
        _isDemo: true,
      };
    }
    ...
    let shorts = [];
    try {
      shorts = await _genShortsWithRetry(sfPrompt);
    } catch (e) {
  ...
  genShortsOnly: async (ana, sty, styPrompt, { signal } = {}) => {
    if (!hasKey('llm')) {
      await wait(2500);
      return { longform: null, shorts: [], _isDemo: true };
    }
    const styleBlock = PROMPT.buildStyleBlock(sty, styPrompt);
    const anaBlock = PROMPT.buildAnaBlock(ana);
    const transcriptBlock =
      S.video.transcript && S.video.transcript.length > 50
  ...
  factCheck: async (sc, { signal } = {}) => {
    ...
    if (fcResult == null)
      throw new Error('팩트체크 AI 응답을 파싱할 수 없습니다. 다시 시도해주세요.');
    return fcResult;
  },
  ...
  extractKw: async (sc, { signal } = {}) => {
    ...
    if (ekwResult == null)
      throw new Error('풋티지 브리프 AI 응답을 파싱할 수 없습니다. 다시 시도해주세요.');
    return ekwResult;
  },
  ...
  genVoice: (text, voiceId) => {
    return wait(2000).then(() => {
      return M.voice;
    });
  },
  genThumb: (title, script) => {
