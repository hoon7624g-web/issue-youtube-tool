...
export function isNews(name) {
  return NEWS_CH.some((n) => {
    return name.indexOf(n) !== -1;
  });
}
export function isBreaking(title) {
  return BREAKING_KW.some((k) => {
    return title.indexOf(k) !== -1;
  });
}
export function isPlanned(title) {
  return PLANNED_KW.some((k) => {
    return title.indexOf(k) !== -1;
  });
}
...
export function scoreVids(vids) {
  ...
  return vids
    .map((v) => {
      ...
      const news = isNews(v.ch),
        breaking = isBreaking(v.title);
...
export function extractJSON(raw) {
  ...
  for (const s of candidates) {
    ...
    if (end > s) {
      try {
        return JSON.parse(clean.substring(s, end));
      } catch (e) {
        /* 다음 후보로 */
      }
    }
