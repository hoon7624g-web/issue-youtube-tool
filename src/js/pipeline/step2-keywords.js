...
function syncSelectedKw() {
  ...
  selected.forEach((kw) => {
    const span = el('span', {
      className: 'tag on',
      style:
        'padding:6px 12px;font-size:12px;cursor:pointer;display:inline-flex;align-items:center;gap:6px',
    });
  ...
  tags.onclick = (e) => {
    const t = e.target.closest('[data-rmkw]');
    if (!t) return;
    const rmId = t.dataset.rmkw;
    const updated = (S.search.skw || []).filter((k) => {
      return k.id !== rmId;
    });
  ...
  if (guide) {
    if (selected.length === 0) guide.textContent = '💡 보통 1~3개 선택을 추천합니다';
      ...
      guide.textContent = '⚠️ 키워드가 많으면 검색 결과가 흐려질 수 있어요';
...
function addCustomKw() {
  const inp = $('customKwInput');
  if (!inp) return;
  const val = inp.value.trim();
  if (!val) {
    toast('키워드를 입력해주세요', 'err');
    return;
  }
  if (
    (S.search.skw || []).some((k) => {
...
registerStep(2, () => {
  ...
  p.appendChild(
    el('p', {
      className: 'pd',
      textContent: '실시간 이슈 키워드를 선택하세요. 여러 개를 선택할 수 있습니다.',
    })
  );

  // 이슈링크 + Google Trends 로딩 (2컬럼 그리드)
  const loadGrid = el('div', {
  ...
  ilLoad.appendChild(
    el('div', { style: 'font-size:13px;color:var(--t3)', textContent: '이슈링크 불러오는 중...' })
  );
  ...
  kwInput.addEventListener('keydown', (e) => {
    if (e.key === 'Enter' && !e.isComposing) addCustomKw();
  });
  ...
  p.appendChild(
    el('div', { id: 'customKwTags', className: 'fx-wrap-6', style: 'margin-top:10px' })
  );
  ...
  p.appendChild(
    el('div', {
      id: 'kwGuide',
      style: 'font-size:12px;color:var(--t4);margin-top:8px;min-height:18px',
      textContent: '\uD83D\uDCA1 보통 1~3개 선택을 추천합니다',
    })
  );
  ...
  navRow.appendChild(
    el('span', {
      id: 'kc',
      style: 'font-size:13px;color:var(--t3);font-weight:500',
      textContent: '선택: 0개',
    })
  );
  const nxtBtn = el('button', {
  ...
  function setupKwClick(container) {
    container.onclick = (e) => {
      const t = e.target.closest('[data-kwid]');
      if (!t) return;
  ...
  Promise.all([Api.getIssueLink(), Api.getTrends()]).then(([data, keywords]) => {
    ...
    const ilCard = el('div', {
      className: 'cd',
      style: 'border-color:var(--acc-ring);padding:0;overflow:hidden',
    });
    const ilHdr = el('div', {
      style:
        'padding:14px 18px;background:var(--acc-bg);border-bottom:1px solid var(--acc-ring);display:flex;align-items:center;gap:8px',
    });
    ilHdr.appendChild(
      el('span', {
        style:
          'width:8px;height:8px;border-radius:50%;background:var(--acc);display:inline-block;animation:pulse 1.5s infinite',
      })
    );
    ilHdr.appendChild(
      el('span', {
        style: 'font-size:14px;font-weight:700;color:var(--acc)',
        textContent: isGoogleFallback ? '실시간 뉴스 핫이슈' : '이슈링크 핫이슈',
      })
    );
    ilHdr.appendChild(
      el('span', {
        style:
          'font-size:10px;font-weight:600;color:#fff;background:var(--acc);padding:2px 8px;border-radius:10px;margin-left:auto',
        textContent: 'LIVE',
      })
    );
    if (isGoogleFallback) {
      ilHdr.appendChild(
        el('span', {
          style: 'font-size:9px;color:var(--t4);margin-left:4px',
          textContent: '(Google News)',
        })
      );
    }
    ...
    if (data.hotKeywords.length) {
      data.hotKeywords.forEach((k, i) => {
        shared.ilKw['hot-' + i] = {
          id: 'hot-' + i,
          label: k.keyword,
          src: isGoogleFallback ? '실시간 뉴스' : '이슈링크 핫이슈',
          score: 100 - i * 5,
          tags: [],
          period: 'weekly',
        };
        ...
        if (i < 3) {
          row.appendChild(
            el('span', {
              style:
                'font-size:9px;padding:1px 6px;border-radius:4px;background:var(--acc-bg2);color:var(--acc);font-weight:700;flex-shrink:0',
              textContent: '\uD83D\uDD25 추천',
            })
          );
        }
        ...
        if (cat)
          row.appendChild(el('span', { className: 'kw-label ' + cat.cls, textContent: cat.text }));
        ilList.appendChild(row);
      });
    } else {
      ilList.appendChild(
        el('div', {
          style: 'padding:24px;text-align:center;color:var(--t4);font-size:13px',
          textContent: '이슈링크 데이터를 불러올 수 없습니다',
        })
      );
    }
    ...
    if (zumKws.length > 0) {
      const zumCard = el('div', {
        className: 'cd',
        style: 'border-color:rgba(37,99,235,.3);padding:0;overflow:hidden',
      });
      const zumHdr = el('div', {
        style:
          'padding:14px 18px;background:rgba(37,99,235,.04);border-bottom:1px solid rgba(37,99,235,.2);display:flex;align-items:center;gap:8px',
      });
      zumHdr.appendChild(
        el('span', {
          style:
            'width:8px;height:8px;border-radius:50%;background:#2563EB;display:inline-block;animation:pulse 1.5s infinite',
        })
      );
      zumHdr.appendChild(
        el('span', {
          style: 'font-size:14px;font-weight:700;color:#2563EB',
          textContent: '줌 실시간 검색어',
        })
      );
      zumHdr.appendChild(
        el('span', {
          style:
            'font-size:10px;font-weight:600;color:#fff;background:#2563EB;padding:2px 8px;border-radius:10px;margin-left:auto',
          textContent: 'LIVE',
        })
      );
      ...
      zumKws.forEach((k, i) => {
        ...
        row.appendChild(
          el('span', {
            className: 'kw-rank ' + _rankClass(i, 'realtime'),
            textContent: String(i + 1),
          })
        );
        if (i < 3)
          row.appendChild(
            el('span', {
              style:
                'font-size:9px;padding:1px 6px;border-radius:4px;background:rgba(37,99,235,.1);color:#2563EB;font-weight:700;flex-shrink:0',
              textContent: '\uD83D\uDD25 추천',
            })
          );
        row.appendChild(el('span', { className: 'kw-text', textContent: k.keyword }));
        ...
        if (zcat)
          row.appendChild(
            el('span', { className: 'kw-label ' + zcat.cls, textContent: zcat.text })
          );
    ...
    // ═══ 네이트 실시간 검색어 ═══
    if (nateKws.length > 0) {
      const nateCard = el('div', {
        className: 'cd',
        style: 'border-color:rgba(249,115,22,.3);padding:0;overflow:hidden',
      });
      const nateHdr = el('div', {
        style:
          'padding:14px 18px;background:rgba(249,115,22,.04);border-bottom:1px solid rgba(249,115,22,.2);display:flex;align-items:center;gap:8px',
      });
      nateHdr.appendChild(
        el('span', {
          style:
            'width:8px;height:8px;border-radius:50%;background:#F97316;display:inline-block;animation:pulse 1.5s infinite',
        })
      );
      nateHdr.appendChild(
        el('span', {
          style: 'font-size:14px;font-weight:700;color:#F97316',
          textContent: '네이트 실시간 검색어',
        })
      );
      nateHdr.appendChild(
        el('span', {
          style:
            'font-size:10px;font-weight:600;color:#fff;background:#F97316;padding:2px 8px;border-radius:10px;margin-left:auto',
          textContent: 'LIVE',
        })
      );
      ...
      nateKws.forEach((k, i) => {
        ...
        if (i < 3)
          row.appendChild(
            el('span', {
              style:
                'font-size:9px;padding:1px 6px;border-radius:4px;background:rgba(249,115,22,.1);color:#F97316;font-weight:700;flex-shrink:0',
              textContent: '\uD83D\uDD25 추천',
            })
          );
        row.appendChild(el('span', { className: 'kw-text', textContent: k.keyword }));
        ...
        if (ncat)
          row.appendChild(
            el('span', { className: 'kw-label ' + ncat.cls, textContent: ncat.text })
          );
    ...
    if (googleKws.length > 0) {
      const googleCard = el('div', {
        className: 'cd',
        style: 'border-color:rgba(52,168,83,.3);padding:0;overflow:hidden',
      });
      const gHdr = el('div', {
        style:
          'padding:14px 18px;background:rgba(52,168,83,.04);border-bottom:1px solid rgba(52,168,83,.2);display:flex;align-items:center;gap:8px',
      });
      gHdr.appendChild(
        el('span', {
          style:
            'width:8px;height:8px;border-radius:50%;background:#34A853;display:inline-block;animation:pulse 1.5s infinite',
        })
      );
      gHdr.appendChild(
        el('span', {
          style: 'font-size:14px;font-weight:700;color:#34A853',
          textContent: '구글 트렌드',
        })
      );
      gHdr.appendChild(
        el('span', {
          style:
            'font-size:10px;font-weight:600;color:#fff;background:#34A853;padding:2px 8px;border-radius:10px;margin-left:auto',
          textContent: 'LIVE',
        })
      );
      ...
      googleKws.forEach((k, i) => {
        ...
        if (i < 3)
          row.appendChild(
            el('span', {
              style:
                'font-size:9px;padding:1px 6px;border-radius:4px;background:rgba(52,168,83,.1);color:#34A853;font-weight:700;flex-shrink:0',
              textContent: '\uD83D\uDD25 추천',
            })
          );
        row.appendChild(el('span', { className: 'kw-text', textContent: k.keyword }));
        ...
        if (gcat)
          row.appendChild(
            el('span', { className: 'kw-label ' + gcat.cls, textContent: gcat.text })
          );
    ...
    if (zumKws.length === 0 && nateKws.length === 0 && googleKws.length === 0) {
      const emptyCard = el('div', {
        className: 'cd',
        style: 'border-color:var(--bdr);padding:0;overflow:hidden;grid-column:span 3',
      });
      emptyCard.appendChild(
        el('div', {
          style: 'padding:24px;text-align:center;color:var(--t4);font-size:13px',
          textContent: '실시간 검색어 데이터를 불러올 수 없습니다',
        })
      );
