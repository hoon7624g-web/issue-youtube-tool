...
sOn(K.NAV_STEP, (step) => {
  if (step !== 6) {
    ...
    try {
      _scriptLeaveAC.abort(new Error('step-leave'));
    } catch (e) {}
...
function _syncFcBtn() {
  const btn = $('s6b');
  if (!btn) return;
  const any = Object.keys(_selected).some((k) => {
    return _selected[k];
  });
  btn.disabled = !any;
  btn.style.opacity = any ? '1' : '.4';
  const cnt = Object.keys(_selected).filter((k) => {
    return _selected[k];
  }).length;
  ...
  if (hint) {
    ...
    if (cnt > 0) {
      hint.style.cssText =
        'text-align:center;font-size:12px;margin-top:8px;padding:10px 16px;background:var(--acc-bg);border:1px solid var(--acc-ring);border-radius:var(--r);color:var(--t2)';
      const parts = [];
...
function _commitSelection() {
  ...
  const results = sel.map((s) => {
    return { script: s, fcs: null, ekw: null, voiceResult: null };
  });
  sSet({ [K.SCRIPT_SELECTED]: sel, [K.SCRIPT_RESULTS]: results, [K.SCRIPT_CUR_IDX]: 0 });
  if (sel.length) {
    sSet({
      [K.SCRIPT_SCR]: { title: sel[0].title, content: sel[0].content },
      [K.SCRIPT_ES]: sel[0].content,
    });
  }
}
  ...
  container.querySelectorAll('.script-sel').forEach((selEl) => {
    selEl.addEventListener('click', (e) => {
      ...
      if (_selected[key]) {
        ...
        if (check) {
          check.style.borderColor = key === 'longform' ? '#2563EB' : '#DC2626';
          check.style.background = key === 'longform' ? '#2563EB' : '#DC2626';
          check.textContent = '\u2713';
        ...
        if (check) {
          check.style.borderColor = 'var(--bdr)';
...
function _buildShortItem(s, i) {
  const item = el('div', {
    className: 'short-item script-sel',
    style:
      'padding:12px 14px;margin-bottom:8px;background:var(--bg);border-radius:var(--r);cursor:pointer;border:1.5px solid var(--bdr);transition:all .2s',
  });
  ...
  row.appendChild(
    el('div', {
      className: 'sel-check',
      style:
        'width:22px;height:22px;border-radius:6px;border:2px solid var(--bdr);display:flex;align-items:center;justify-content:center;font-size:14px;flex-shrink:0;transition:all .2s',
    })
  );
  row.appendChild(
    el('div', {
      style: 'flex:1;font-size:13px;font-weight:600;line-height:1.4',
      textContent: s.title || '',
    })
  );
  const charSpan = el('span', {
    className: 't-2xs-t4 short-char-count',
    textContent: s.content.length + '자',
  });
  ...
  const editor = el('textarea', {
    ...
    style:
      'display:none;margin-top:10px;font-size:12px;line-height:1.7;min-height:120px;max-height:250px;resize:vertical;white-space:pre-wrap',
  });
...
function _showRetryError(msg) {
  ...
  retryBtn.addEventListener('click', () => {
    sSet({ [K.SCRIPT_SCR_DUAL]: null, [K.SCRIPT_SCR]: null });
    runStep(6);
  });
...
function _buildLongformCard(lf) {
  ...
  const lfCard = el('div', {
    className: 'cd cd-bar-blue script-sel',
    style: 'border-left:4px solid #2563EB;cursor:pointer;transition:all .2s;margin-bottom:0',
  });
  lfCard.dataset.key = 'longform';

  const lfHeader = el('div', { className: 'cdh mb-12' });
  const lfHeaderLeft = el('div', { className: 'fx-row' });
  lfHeaderLeft.appendChild(
    el('div', {
      className: 'sel-check',
      style:
        'width:22px;height:22px;border-radius:6px;border:2px solid var(--bdr);display:flex;align-items:center;justify-content:center;font-size:14px;transition:all .2s',
    })
  );
  lfHeaderLeft.appendChild(
    el('span', { className: 'badge-longform', textContent: '롱폼 ~' + lfCharMin + '분' })
  );
  lfHeader.appendChild(lfHeaderLeft);

  const lfCopyBtn = el('button', {
    className: 'btn bs',
  ...
  lfCopyBtn.addEventListener('click', (e) => {
    e.stopPropagation();
    navigator.clipboard.writeText(S.script.scrDual.longform.content).then(() => {
      toast('롱폼 대본 복사됨');
    });
  });
  ...
  lfCard.appendChild(
    el('div', {
      style: 'font-size:16px;font-weight:700;margin-bottom:12px;line-height:1.4',
      textContent: lf.title || '',
    })
  );

  const lfEditor = el('textarea', {
    className: 'inp',
    id: 'lfEditor',
    style:
      'font-size:13px;line-height:1.8;min-height:300px;max-height:500px;resize:vertical;white-space:pre-wrap',
  });
  lfEditor.value = lf.content;
  lfEditor.addEventListener('click', (e) => {
    e.stopPropagation();
  });
  lfEditor.addEventListener('input', () => {
    if (S.script.scrDual) {
      const updated = Object.assign({}, S.script.scrDual, {
        longform: Object.assign({}, S.script.scrDual.longform, { content: lfEditor.value }),
      });
  ...
  lfCard.appendChild(
    el('div', {
      id: 'lfCount',
      style: 'margin-top:12px;font-size:11px;color:var(--t4)',
      textContent: lf.content.length + '자',
    })
  );
  return lfCard;
}
...
function _buildShortsCard(shorts) {
  const sfCard = el('div', {
    className: 'cd cd-bar-red',
    style: 'border-left:4px solid #DC2626;margin-bottom:0',
  });
  sfCard.appendChild(
    el('div', { className: 'mb-14' }, [
      el('span', {
        style:
          'font-size:11px;font-weight:600;color:#DC2626;background:rgba(220,38,38,.1);padding:3px 8px;border-radius:4px',
        textContent: '숏폼 30초~1분 \u00D7 ' + shorts.length + '개',
      }),
    ])
  );
  if (shorts.length) {
    shorts.forEach((s, i) => {
      sfCard.appendChild(_buildShortItem(s, i));
    });
  } else {
    sfCard.appendChild(
      el('div', {
        style: 'padding:20px;text-align:center;color:var(--t3);font-size:13px',
        textContent: '숏폼 대본이 생성되지 않았습니다.',
      })
    );
  }
  return sfCard;
...
function renderDualScript(dual, styles) {
  ...
  root.appendChild(
    el('p', { className: 'pd', textContent: '진행할 스크립트를 선택하세요 (복수 선택 가능)' })
  );
  ...
  const guideBox = el('div', {
    id: 's6Guide',
    style:
      'padding:14px 18px;background:rgba(37,99,235,.06);border:1px solid rgba(37,99,235,.15);border-radius:var(--r2);margin-bottom:16px;display:flex;align-items:center;gap:10px',
  });
  ...
  guideInner.appendChild(
    el('strong', { textContent: '아래 스크립트 카드를 클릭하여 선택하세요.' })
  );
  guideInner.appendChild(
    document.createTextNode(' 선택한 스크립트만 팩트체크 → 풋티지 → 음성 단계로 진행됩니다.')
  );
  guideBox.appendChild(guideInner);
  root.appendChild(guideBox);

  // 안내 힌트
  const modeHint = el('div', {
    id: 's6modeHint',
    style: 'font-size:12px;color:var(--t3);margin-bottom:16px',
    textContent: '체크박스로 스크립트를 선택하고, 텍스트를 직접 클릭하여 편집할 수 있습니다.',
  });
  ...
  regenBtn.addEventListener('click', () => {
    // 현재 대본을 히스토리에 저장
    const current =
      S.script.scrDual || (S.script.scr ? { longform: S.script.scr, shorts: [] } : null);
  ...
  if (history.length > 0) {
    ...
    const histBtn = el('button', {
      className: 'btn btn-o',
      textContent: '이전 버전 (' + history.length + ')',
    });
    const histDropdown = el('div', {
      style:
        'display:none;position:absolute;top:100%;left:0;z-index:100;background:var(--white);border:1px solid var(--bdr);border-radius:var(--r2);box-shadow:var(--shadow-md);min-width:280px;max-width:380px;margin-top:6px;padding:8px 0;overflow:hidden',
    });
    history.forEach((h, i) => {
      const item = el('div', {
        style:
          'padding:10px 16px;cursor:pointer;font-size:13px;line-height:1.5;transition:background .15s;border-bottom:1px solid var(--bdr)',
      });
      item.appendChild(
        el('div', {
          style: 'font-weight:600;color:var(--t1)',
          textContent: (h.longform && h.longform.title) || '대본 ' + (i + 1),
        })
      );
      item.appendChild(
        el('div', {
          style: 'font-size:11px;color:var(--t4);margin-top:2px',
          textContent: h._savedAt || '',
        })
      );
      ...
      item.addEventListener('click', () => {
        ...
        sSet({
          [K.SCRIPT_SCR_DUAL]: restored,
          [K.SCRIPT_SCR]: null,
          [K.SCRIPT_ES]: restored.longform ? restored.longform.content : '',
        });
  ...
  setTimeout(() => {
    root.querySelectorAll('textarea').forEach((x) => {
      x.readOnly = false;
      x.style.cursor = 'text';
    });
    root.querySelectorAll('.script-sel').forEach((x) => {
...
registerStep(6, () => {
  let styles = [];
  try {
    styles = JSON.parse(localStorage.getItem('yt_a_sty'));
  } catch (e) {}
  ...
  if (S.script.scrDual) {
    renderDualScript(S.script.scrDual, styles);
    return;
  }
  if (S.script.scr) {
    renderDualScript({ longform: S.script.scr, shorts: [] }, styles);
    return;
  }

  const modeLabel = isShortMode
    ? '숏폼(30초~1분) 5개를 생성합니다'
    : '롱폼(20분+) + 숏폼(30초) 5개를 동시에 생성합니다';
  ...
  genBtn.addEventListener('click', () => {
    ...
    if (isShortMode) {
      ...
      const prog = createProgress(
        'scrLoad',
        'AI 숏폼 대본 작성',
        ['분석 결과 반영', '숏폼 대본 5개 생성'],
        30
      );
      let stepCount = 0;
      const stepTimer = setInterval(() => {
        prog.nextStep();
        if (stepCount === 0) prog.updateMessage('AI 응답 대기 중...');
        else prog.updateMessage('대본 생성 중...');
        stepCount++;
      }, 10000);
      withTimeout(
        (signal) =>
          Api.genShortsOnly(S.analysis.ana, styName, styPrompt, {
            signal: mergeAbortSignals(signal, _scriptLeaveAC.signal),
          }),
        TIMEOUT.SCRIPT_SHORTS,
        '대본 생성 시간이 초과되었습니다.'
      )
        .then((r) => {
      ...
      const liveCard = el('div', {
        className: 'cd cd-bar-blue',
        style: 'border-left:4px solid #2563EB',
      });
      const liveHeader = el('div', {
        style: 'display:flex;align-items:center;justify-content:space-between;margin-bottom:12px',
      });
      const liveHeaderLeft = el('div', { className: 'fx-row' });
      liveHeaderLeft.appendChild(
        el('span', { className: 'badge-longform', textContent: '롱폼 실시간 생성 중' })
      );
      const liveDot = el('span', {
        className: 'glow-pulse',
        style: 'display:inline-block;width:8px;height:8px;border-radius:50%;background:var(--acc)',
      });
      liveHeaderLeft.appendChild(liveDot);
      const liveBadge = liveHeaderLeft.querySelector('span');
      liveHeader.appendChild(liveHeaderLeft);
      const liveCount = el('span', {
        style: 'font-size:11px;color:var(--t4);font-family:var(--mono,monospace)',
        textContent: '0자',
      });
      ...
      const liveArea = el('div', {
        ...
        style:
          'font-size:13px;line-height:1.8;min-height:200px;max-height:400px;overflow-y:auto;white-space:pre-wrap;color:var(--t1);padding:12px;background:var(--bg);border-radius:var(--r);border:1px solid var(--bdr)',
      });
            ...
            (longform) => {
              ...
              if (sfWrap) {
                ...
                const sfCard = el('div', {
                  className: 'cd',
                  style:
                    'border-left:4px solid #DC2626;padding:16px 20px;display:flex;align-items:center;gap:12px',
                });
                sfCard.appendChild(
                  el('div', {
                    className: 'sp',
                    style: 'width:20px;height:20px;border-width:2px;flex-shrink:0',
                  })
                );
                sfCard.appendChild(el('div', { style: 'flex:1' }));
                sfCard.lastChild.appendChild(
                  el('div', {
                    style: 'font-size:13px;font-weight:600;color:#DC2626',
                    textContent: '숏폼 대본 5개 생성 중...',
                  })
                );
                sfCard.lastChild.appendChild(
                  el('div', {
                    className: 'note-xs',
                    textContent: '롱폼 기반으로 다른 앵글의 숏폼을 제작합니다',
                  })
                );
        ...
        .then((r) => {
          ...
          if (sfWrap && sfWrap.style.display !== 'none') {
            ...
            const doneCard = el('div', {
              style:
                'padding:12px 16px;background:rgba(16,185,129,.06);border:1px solid rgba(16,185,129,.15);border-radius:var(--r2);display:flex;align-items:center;gap:8px',
            });
            doneCard.appendChild(
              el('span', { style: 'color:#10B981;font-size:14px', textContent: '✓' })
            );
            doneCard.appendChild(
              el('span', {
                style: 'font-size:13px;font-weight:600;color:#10B981',
                textContent: '숏폼 ' + (r.shorts ? r.shorts.length : 0) + '개 생성 완료',
              })
            );
          ...
          setTimeout(() => {
            ...
            sSet({
              [K.SCRIPT_SCR_DUAL]: r,
              [K.SCRIPT_SCR]: r.longform,
              [K.SCRIPT_ES]: r.longform.content,
            });
            genBtn.disabled = false;
            genBtn.textContent = '다시 생성';
            renderDualScript(r, styles);
