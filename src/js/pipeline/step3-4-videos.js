  ...
  if (hasYtKey()) {
    try {
      ...
      const data = await ytFetch(
        'videos',
        { part: 'snippet,statistics', id: videoId, maxResults: '1' },
        { signal }
      );
      if (data && data.items && data.items.length > 0) {
        ...
        return {
          ...
          subs: 0,
          desc: (snippet.description || '').substring(0, 200),
          score: 0,
          news: false,
        };
      }
    } catch (e) {
      /* YouTube API 실패 → oEmbed fallback */
    }
  }
...
const loadManualUrl = () => {
  ...
  if (!match) {
    if (errEl) {
      errEl.textContent = '유효한 YouTube URL이 아닙니다';
  ...
  _fetchVideoInfo(videoId, signal)
    ...
    .catch((e) => {
      ...
      if (errEl) {
        ...
        errEl.style.display = 'block';
        errEl.style.cssText =
          'display:block;margin-top:8px;padding:12px;background:var(--yel-bg);border:1px solid rgba(184,138,0,.2);border-radius:var(--r);color:var(--t1)';
        const msg = el('div', {
          style: 'font-size:12px;color:var(--yel);margin-bottom:8px',
          textContent: '영상 정보를 가져올 수 없습니다: ' + errMsg,
        });
        errEl.appendChild(msg);
        const proceedBtn = el('button', {
          className: 'btn bs',
          style: 'font-size:12px;padding:6px 14px',
          textContent: '그래도 이 영상으로 진행 →',
        });
...
registerStep(3, () => {
  ...
  p.appendChild(
    el('p', {
      className: 'pd',
      textContent: '잘팔린 컨텐츠를 점수순으로 정렬했습니다. 클릭하여 선택하세요.',
    })
  );

  // 필터 버튼
  const filterRow = el('div', {
    style: 'display:flex;gap:6px;margin-bottom:16px;align-items:center;flex-wrap:wrap',
  });
  const filters = [
    { key: 'long', label: '\uD83C\uDFAC 롱폼' },
    { key: 'short', label: '\uD83D\uDCF1 숏폼' },
  ...
  filterRow.appendChild(
    el('span', { style: 'width:1px;height:20px;background:var(--bdr);margin:0 6px' })
  );
  ...
  filterRow.appendChild(
    el('span', { style: 'font-size:11px;color:var(--t4)', textContent: '이내 영상' })
  );
  ...
  const urlCard = el('div', {
    style:
      'margin-bottom:14px;padding:12px 16px;background:var(--bg);border:1px solid var(--bdr);border-radius:var(--r2);display:flex;align-items:center;gap:10px;flex-wrap:wrap',
  });
  urlCard.appendChild(
    el('span', { style: 'font-size:14px;flex-shrink:0', textContent: '\uD83C\uDFA5' })
  );
  urlCard.appendChild(
    el('span', {
      style: 'font-size:13px;color:var(--t2);flex-shrink:0;font-weight:500',
      textContent: '분석할 영상이 있나요?',
    })
  );
  ...
  const urlGoBtn = el('button', {
    className: 'btn bp',
    style: 'flex-shrink:0;padding:8px 16px',
    textContent: '분석하기',
  });
  ...
  const urlErr = el('div', {
    id: 'manualUrlErr',
    style: 'width:100%;color:var(--red);font-size:12px;display:none',
  });
...
registerStep(4, () => {
  ...
  card.appendChild(
    el('h3', {
      style: 'font-size:18px;font-weight:700;margin-bottom:8px;letter-spacing:-.3px',
      textContent: v.title || '',
    })
  );
  card.appendChild(
    el('div', {
      style: 'font-size:14px;color:var(--t2);margin-bottom:14px;font-weight:500',
  ...
  if (v.views === 0 && v.subs === 0 && v.score === 0) {
    card.appendChild(
      el('div', {
        style:
          'font-size:11px;color:var(--t3);margin-top:12px;padding:8px 12px;background:var(--bg);border-radius:var(--r);line-height:1.5',
        textContent:
          'ℹ️ URL 직접 입력 영상이라 통계 데이터가 제한됩니다. 영상 분석은 정상적으로 진행됩니다.',
      })
    );
  }
  p.appendChild(card);
  ...
  const nextBtn = el('button', {
    className: 'btn bp btn-lg mt-20',
    textContent: '영상 분석 시작 \u2192',
  });
