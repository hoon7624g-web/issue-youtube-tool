...
const GOALS = [
  {
    id: 'script_only',
    icon: '📝',
    title: '대본만 빠르게 뽑고 싶어요',
    desc: 'YouTube + AI 모델 키만 있으면 됩니다',
    keys: ['youtube', 'llm', 'googleAiStudio'],
    color: '#2563EB',
  },
  {
    id: 'with_verify',
    icon: '✅',
    title: '팩트 검증까지 하고 싶어요',
    desc: '+ Perplexity 키를 추가하면 검증 품질 UP',
    keys: ['youtube', 'llm', 'googleAiStudio', 'perplexity'],
    color: '#059669',
  },
  {
    id: 'with_voice',
    icon: '🔊',
    title: '음성까지 만들고 싶어요',
    desc: '+ TTS 키로 AI 음성을 생성합니다',
    keys: ['youtube', 'llm', 'googleAiStudio', 'tts', 'elevenlabs'],
    color: '#7C3AED',
  },
  {
    id: 'full',
    icon: '🎬',
    title: '소스영상까지 자동으로 받고 싶어요',
    desc: '모든 기능을 활용합니다',
    keys: ['youtube', 'llm', 'googleAiStudio', 'tts', 'elevenlabs', 'pexels', 'perplexity'],
    color: 'var(--acc)',
  },
];
...
function _showGoalSelector() {
  ...
  p2.appendChild(
    el('p', {
      className: 'pd',
      textContent: '목표에 따라 필요한 API 키가 달라집니다. 나중에 언제든 추가할 수 있어요.',
    })
  );
  const grid = el('div', {
    style: 'display:grid;grid-template-columns:1fr 1fr;gap:12px;max-width:640px',
  });
  GOALS.forEach((g) => {
    const card = el('div', {
      className: 'cd',
      style: 'cursor:pointer;padding:20px;border-left:4px solid ' + g.color + ';transition:all .2s',
    });
    ...
    header.appendChild(
      el('span', { style: 'font-size:14px;font-weight:700;color:var(--t1)', textContent: g.title })
    );
    card.appendChild(header);
    card.appendChild(
      el('div', { style: 'font-size:12px;color:var(--t3);line-height:1.5', textContent: g.desc })
    );
    const keyCount = g.keys.filter((k) => k !== 'llm').length;
    card.appendChild(
      el('div', {
        style: 'margin-top:10px;font-size:11px;font-weight:600;color:' + g.color,
        textContent: 'API 키 ' + keyCount + '개 필요',
      })
    );
    card.addEventListener('click', () => {
      _selectedGoal = g.id;
      _renderApiKeyForm($('p2'), '저장하고 시작하기 \u2192', () => {
        runStep(2);
      });
  ...
  const skipBtn = el('button', {
    className: 'btn bg t-sm-t3',
    textContent: '목표 없이 직접 설정하기 →',
  });
...
function _highlightGoalFields(goal) {
  ...
  if (needsOptional) {
    ...
    if (optBody && optBody.style.display === 'none') {
      ...
      if (optToggle) {
        ...
        if (label) label.textContent = '고급 설정 접기';
      }
    }
  }
  const fieldMap = {
  ...
  optionalKeys.forEach((k) => {
    ...
    if (goal.keys.includes(k)) {
      ...
      const badge = el('span', {
        style:
          'font-size:10px;font-weight:600;padding:2px 8px;border-radius:4px;background:' +
          goal.color +
          '15;color:' +
          goal.color +
          ';margin-left:6px',
        textContent: '← 이 목표에 필요',
      });
...
registerAction('openMySettings', () => {
  ...
  msBackTop.addEventListener('click', () => {
    syncSb();
    runAction('showP');
  });
  ...
  wrap.appendChild(
    el('p', { className: 'pd', textContent: '내 목소리를 등록하면 AI가 유사한 음성을 생성합니다' })
  );
  ...
  const voiceLabel = el('label', {
    id: 'voiceUploadLabel',
    style:
      'display:inline-flex;align-items:center;gap:8px;padding:12px 20px;border-radius:var(--r2);border:1.5px dashed var(--bdr2);cursor:pointer;color:var(--t2);font-size:13px;font-weight:500;transition:all .2s;background:var(--bg)',
  });
  ...
  uploadRow.appendChild(
    el('span', {
      id: 'uploadStatus',
      className: 't-xs-t3',
      textContent: S.voice && S.voice.elVoiceId ? '\u2713 등록됨' : '',
    })
  );
  voiceCard.appendChild(uploadRow);
  voiceCard.appendChild(
    el('p', {
      style: 'font-size:11px;color:var(--t4);line-height:1.6',
      textContent:
        'ElevenLabs API 키가 설정되어야 합니다. 30초~3분 분량의 깨끗한 음성 파일을 권장합니다.',
    })
  );
  voiceCard.appendChild(
    el('p', {
      style:
        'font-size:10px;color:var(--t4);line-height:1.5;margin-top:6px;padding-top:6px;border-top:1px solid var(--bdr)',
      textContent:
        '\uD83D\uDD12 음성 파일은 ElevenLabs 서버로 전송되어 AI 음성 모델이 생성됩니다. 회사 서버에는 저장되지 않습니다.',
    })
  );
  ...
  speedRow.appendChild(
    el('span', {
      ...
      textContent: (S.voice ? (S.voice.voiceSpeed || 1.0).toFixed(1) : '1.0') + 'x',
    })
  );
  ...
  p2.appendChild(wrap);
  voiceLabel.addEventListener('mouseenter', () => {
    voiceLabel.style.borderColor = '#8B5CF6';
  });
  voiceLabel.addEventListener('mouseleave', () => {
    voiceLabel.style.borderColor = 'var(--bdr2)';
  });
  fileInput.addEventListener('change', () => {
    runAction('handleVoiceUpload', fileInput);
  });
  slider.addEventListener('input', () => {
    const v = parseFloat(slider.value);
    sSet({ 'voice.voiceSpeed': v });
    $('msSpeedValue').textContent = v.toFixed(1) + 'x';
  });
  backBtn.addEventListener('click', () => {
    syncSb();
    runAction('showP');
