...
sOn(K.NAV_STEP, (step) => {
  if (step !== 5) {
    ...
    try {
      _analysisLeaveAC.abort(new Error('step-leave'));
    } catch (e) {}
...
function rAna() {
  ...
  const methodMap = {
    video: {
      text: '\uD83C\uDFAC AI Studio 영상 분석',
      color: '#4285F4',
      bg: 'rgba(66,133,244,.08)',
    },
    subtitle: { text: '\uD83D\uDCDD 자막 기반 분석', color: '#D97706', bg: 'rgba(217,119,6,.08)' },
    title_only: {
      text: '\uD83D\uDCCB 제목/설명 기반 분석',
      color: '#6B7280',
  ...
  root.appendChild(
    el('div', {
      style:
        'margin-bottom:16px;display:inline-flex;align-items:center;gap:6px;font-size:12px;font-weight:600;color:' +
        m.color +
        ';background:' +
        m.bg +
  ...
  if (a._usedFallback) {
    const fallbackNote = el('div', {
      style:
        'margin:-4px 0 16px;padding:10px 12px;border:1px solid rgba(217,119,6,.18);background:rgba(245,158,11,.06);border-radius:10px',
    });
    fallbackNote.appendChild(
      el('div', {
        style: 'font-size:12px;font-weight:600;color:#B45309;margin-bottom:4px',
        textContent: 'AI Studio 직접 분석 중 오류가 발생해 대체 분석으로 전환되었습니다.',
      })
    );
    fallbackNote.appendChild(
      el('div', {
        style: 'font-size:12px;color:var(--t3);line-height:1.6',
        textContent:
          a._method === 'subtitle'
            ? '저장된 자막을 활용해 분석을 이어갔습니다.'
            : '자막이 없어 제목/설명 기반으로 분석을 이어갔습니다.',
      })
    );
    if (a._fallbackReason) {
      fallbackNote.appendChild(
        el('div', {
          style: 'font-size:11px;color:var(--t4);margin-top:6px;word-break:break-word',
          textContent: '원인: ' + a._fallbackReason,
        })
      );
    }
  ...
  const sumBody = el('p', {
    style: 'font-size:14px;line-height:1.9;color:var(--t2)',
  ...
  grid.appendChild(
    Card({ barColor: 'green', children: [reasonHdr, AccentList(a.reasons, 'al-grn')] })
  );
  ...
  structHdr.appendChild(
    CopyButton(() => (a.structure || []).map((s, i) => i + 1 + '. ' + s).join('\n'))
  );
  ...
  stickyBar.appendChild(
    el('span', { className: 't-xs-t3', textContent: '분석 완료 · 다음 단계에서 대본을 생성합니다' })
  );
  const nextBtn = el('button', {
    className: 'btn bp btn-lg',
    textContent: '스크립트 생성하기 \u2192',
  });
...
function startAnalysis(transcript, skipAIStudio) {
  ...
  withTimeout(
    (signal) => {
      ...
      return skipAIStudio
        ? Api._analyzeFallback(S.video.sv, transcript, { signal: taskSignal })
        : Api.analyze(S.video.sv, transcript, { signal: taskSignal });
    },
    TIMEOUT.ANALYSIS,
    'AI 분석 시간이 초과되었습니다.'
  )
...
function showSubtitleInput(errorMsg) {
  ...
  card.appendChild(
    el('div', {
      style: 'font-size:14px;font-weight:600;color:var(--t1);margin-bottom:12px',
      textContent: errorMsg ? '자동 자막 추출 실패' : '자막을 입력하면 더 정확한 분석이 가능합니다',
    })
  );
  if (errorMsg) {
    card.appendChild(
      el('div', {
        style: 'font-size:12px;color:var(--t3);margin-bottom:12px',
  ...
  const guide = el('div', {
    style:
      'font-size:13px;color:var(--t2);margin-bottom:16px;line-height:1.8;background:var(--bg);padding:14px 16px;border-radius:var(--r);border:1px solid var(--bdr)',
  });
  guide.appendChild(
    el('div', {
      style: 'font-weight:600;margin-bottom:8px;color:var(--t1)',
      textContent: 'YouTube에서 자막 복사하는 방법',
    })
  );
  const steps = [
    '① YouTube 영상 페이지 열기',
  ...
  guide.appendChild(
    el('div', {
      style: 'font-size:11px;color:var(--t4);margin-top:8px',
      textContent: '💡 자막이 없는 영상도 "자막 없이 분석"으로 진행할 수 있습니다',
    })
  );
  ...
  const subBtn = el('button', {
    className: 'btn bp btn-lg',
    textContent: '자막 포함 분석 시작 \u2192',
  });
  subBtn.addEventListener('click', () => {
    const manual = $('manualSub').value.trim();
    if (!manual) {
      toast('자막을 붙여넣어주세요', 'err');
      return;
    }
  ...
  const skipBtn = el('button', {
    ...
    style: 'color:var(--t3)',
    textContent: '자막 없이 분석',
  });
  skipBtn.addEventListener('click', () => {
    sSet({ [K.VIDEO_TRANSCRIPT]: '' });
    startAnalysis('', true);
  });
...
const _ls5Fallback = () => {
  ...
  if (isElec) {
    ...
    root.appendChild(ld);
    getCachedSubtitle(S.video.sv.id)
      .then((sub) => {
      ...
      .catch(() => {
        showSubtitleInput('자막을 자동으로 추출하지 못했습니다.');
      });
  } else {
    showSubtitleInput();
  }
...
registerStep(5, () => {
  ...
  if (gaiKey && S.video.sv && S.video.sv.id) {
    ...
    root.appendChild(
      el('p', {
        className: 'pd',
        textContent: 'Google AI Studio가 영상을 직접 시청하고 분석합니다',
      })
    );

    // ── 영상 정보 카드 (대기 중 맥락 제공) ──
    const videoCard = el('div', {
      className: 'cd',
    ...
    const thumbWrap = el('div', {
      style: 'width:220px;flex-shrink:0;position:relative;background:#000',
    });
    if (v.thumb) {
    ...
    const overlay = el('div', {
      style:
        'position:absolute;inset:0;display:flex;flex-direction:column;align-items:center;justify-content:center;background:rgba(0,0,0,.4)',
    });
    const eyeIcon = el('div', {
      style:
        'width:40px;height:40px;border-radius:50%;background:rgba(66,133,244,.9);display:flex;align-items:center;justify-content:center;margin-bottom:6px;animation:pulse 1.5s infinite',
    });
    eyeIcon.appendChild(el('span', { style: 'color:#fff;font-size:18px', textContent: '👁' }));
    overlay.appendChild(eyeIcon);
    overlay.appendChild(
      el('span', {
        style: 'color:#fff;font-size:11px;font-weight:600;text-shadow:0 1px 3px rgba(0,0,0,.5)',
        textContent: 'AI 시청 중',
      })
    );
    ...
    const infoWrap = el('div', {
      style: 'flex:1;padding:18px 20px;display:flex;flex-direction:column;justify-content:center',
    });
    infoWrap.appendChild(
      el('div', {
        style: 'font-size:15px;font-weight:700;color:var(--t1);line-height:1.4;margin-bottom:8px',
        textContent: v.title || '',
      })
    );
    infoWrap.appendChild(
    ...
    if (v.views)
      statRow.appendChild(
        el('span', {
          style:
            'font-size:11px;color:var(--t3);background:var(--bg2);padding:3px 8px;border-radius:4px',
          textContent: '▶ ' + fmt(v.views),
        })
      );
    if (v.subs)
      statRow.appendChild(
    ...
    if (v.score)
      statRow.appendChild(
        el('span', {
          style:
            'font-size:11px;color:var(--acc);background:var(--acc-bg);padding:3px 8px;border-radius:4px;font-weight:600',
          textContent: '점수 ' + v.score,
        })
      );
    if (v.durText)
      statRow.appendChild(
        el('span', {
          style:
            'font-size:11px;color:var(--t3);background:var(--bg2);padding:3px 8px;border-radius:4px;font-family:var(--mono)',
          textContent: v.durText,
        })
      );
    ...
    // 상태 헤더
    const progHeader = el('div', {
      style: 'display:flex;align-items:center;justify-content:space-between;margin-bottom:16px',
    });
    const progHeaderLeft = el('div', { className: 'fx-row' });
    const liveStatusBadge = el('span', {
      style:
        'font-size:12px;font-weight:600;color:#4285F4;background:rgba(66,133,244,.1);padding:4px 10px;border-radius:6px',
      textContent: '영상 분석 중...',
    });
    progHeaderLeft.appendChild(liveStatusBadge);
    const liveDot = el('span', {
      style:
        'display:inline-block;width:8px;height:8px;border-radius:50%;background:#4285F4;animation:pulse 1s infinite',
    });
    progHeaderLeft.appendChild(liveDot);
    progHeader.appendChild(progHeaderLeft);
    const elapsedSpan = el('span', {
      id: '_s5elapsed',
    ...
    stepItems.forEach((s, i) => {
      const row = el('div', {
        style:
          ...
          (i < stepItems.length - 1 ? 'border-bottom:1px solid var(--bdr)' : ''),
      });
      const dot = el('div', {
        id: '_s5dot' + i,
        style:
          'width:28px;height:28px;border-radius:50%;background:var(--bg2);display:flex;align-items:center;justify-content:center;flex-shrink:0;font-size:12px;font-weight:700;color:var(--t4);transition:all .3s',
        textContent: '○',
      });
      row.appendChild(dot);
      const textWrap = el('div', { style: 'flex:1' });
      textWrap.appendChild(
        el('div', { style: 'font-size:13px;font-weight:500;color:var(--t2)', textContent: s.label })
      );
      textWrap.appendChild(
        el('div', { style: 'font-size:11px;color:var(--t4);margin-top:1px', textContent: s.desc })
      );
    ...
    withTimeout(
      ...
      .then((r) => {
        ...
        liveStatusBadge.style.background = usedFallback
          ? 'rgba(217,119,6,.1)'
          : 'rgba(16,185,129,.1)';
        for (let i = 0; i < stepItems.length; i++) {
          const dot = document.getElementById('_s5dot' + i);
        ...
        setTimeout(() => {
          ...
          if (usedFallback) {
            toast(
              actualMethod === 'subtitle'
                ? '영상 직접 분석이 실패하여 자막 기반 분석 결과를 표시합니다.'
                : '영상 직접 분석이 실패하여 제목/설명 기반 분석 결과를 표시합니다.'
            );
          }
          rAna();
        }, 800);
      ...
      .catch((e) => {
        ...
        const notice = el('div', {
          className: 'cd',
          style: 'border-left:4px solid var(--yel);max-width:560px;margin:40px auto',
        });
        notice.appendChild(
          el('div', {
            style: 'font-size:15px;font-weight:700;color:var(--t1);margin-bottom:10px',
            textContent: '영상 직접 분석에 실패했습니다',
          })
        );

        const reasonText = (e && e.message) || '';
        let cause = '네트워크 오류 또는 일시적 서버 문제';
        if (reasonText.includes('키') || reasonText.includes('401') || reasonText.includes('403'))
          cause = 'API 키 권한 부족 또는 키 만료';
        else if (
          reasonText.includes('시간') ||
          reasonText.includes('timeout') ||
          reasonText.includes('Timeout')
        )
          cause = '응답 시간 초과 (영상이 길거나 서버 지연)';
        else if (reasonText.includes('quota') || reasonText.includes('429'))
          cause = 'API 요청 한도 초과';

        notice.appendChild(
          el('div', {
            style: 'font-size:13px;color:var(--t3);margin-bottom:14px;line-height:1.7',
            textContent: '가능한 원인: ' + cause,
          })
        );

        const switchBox = el('div', {
          style:
            'padding:12px 16px;background:rgba(37,99,235,.06);border:1px solid rgba(37,99,235,.12);border-radius:var(--r);line-height:1.7',
        });
        switchBox.appendChild(
          el('div', {
            style: 'font-size:13px;font-weight:600;color:#2563EB;margin-bottom:4px',
            textContent: '🔄 자막 기반 분석으로 자동 전환합니다',
          })
        );
        switchBox.appendChild(
          el('div', {
            className: 't-xs-t2',
            textContent:
              '영상 화면 분석 대신 자막 텍스트를 활용합니다. 정확도는 다소 낮을 수 있지만 대부분 계속 진행 가능합니다.',
          })
        );
