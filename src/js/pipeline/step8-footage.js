...
// ═══════════════════════════════════════
import { $, toast, withTimeout, safeUrl, createProgress , el, LABEL_COLORS, mergeAbortSignals } from '../utils.js';
...
sOn(K.NAV_STEP, (step) => {
  if (step !== 8) {
    _ekRunId++;
    _ekJobRunning = false;
    try { _ekLeaveAC.abort(new Error('step-leave')); } catch (e) {}
...
function sbUrl(q) { return 'https://www.storyblocks.com/video/search/' + encodeURIComponent(q); }
...
function _buildSceneCard(s, i, ri, hasPexels) {
  ...
  const numBadge = el('div', { style: 'width:32px;height:32px;border-radius:8px;background:' + color + '15;display:flex;align-items:center;justify-content:center;flex-shrink:0;font-size:14px;font-weight:700;color:' + color, textContent: String(i + 1) });
  const headerInfo = el('div', { className: 'flex-1' });
  headerInfo.appendChild(el('span', { style: 'font-size:13px;font-weight:700;color:' + color, textContent: s.label || '' }));
  headerInfo.appendChild(el('span', { style: 'font-size:12px;color:var(--t3);margin-left:8px', textContent: s.cut || '' }));
  headerRow.appendChild(numBadge);
  headerRow.appendChild(headerInfo);
  card.appendChild(headerRow);

  // 대사
  const textDiv = el('div', { style: 'font-size:14px;color:var(--t1);line-height:1.6;margin-bottom:10px;padding:10px 14px;background:var(--bg);border-radius:var(--r);font-style:italic' });
  textDiv.textContent = '\u201C' + (s.text || '') + '\u201D';
  card.appendChild(textDiv);

  // 목적
  card.appendChild(el('div', { style: 'font-size:13px;color:var(--t2);margin-bottom:10px', textContent: s.purpose || '' }));
  ...
  function makeKwTag(text, isMain, isBg2) {
    const tag = el('span', {
      ...
      style: 'font-weight:' + (isMain ? '600' : '400') + ';cursor:pointer' + (isBg2 ? ';background:var(--bg2);color:var(--t2)' : ''),
      textContent: '\uD83D\uDD0D ' + text
    });
    tag.addEventListener('click', () => { _loadPexelsRow(pxId, text); });
    return tag;
  }
  ...
  // Pexels 결과 컨테이너
  if (hasPexels) {
    card.appendChild(el('div', { id: pxId, className: 'pexels-row', style: 'display:flex;gap:8px;overflow-x:auto;padding:8px 0' }));
  }
...
function _loadPexelsRow(elId, query) {
  ...
  container.appendChild(el('div', { className: 't-xs-t4', textContent: '"' + query + '" 검색 중...' }));
  ...
  const _handlePexelsResult = (data) => {
    ...
    if (!data.videos || !data.videos.length) {
      ...
      if (safeSb) {
        const sbLink = el('a', { href: safeSb, target: '_blank', rel: 'noopener', style: 'color:var(--acc)', textContent: 'Storyblocks \u2197' });
  ...
  if (window.electronAPI && window.electronAPI.pexelsSearch) {
    pendingPromise = window.electronAPI.pexelsSearch(query).then(r => {
      if (r.status >= 400) { _handlePexelsError(); return; }
    ...
    // 웹 환경 (개발/테스트 전용)
    pendingPromise = fetch('https://api.pexels.com/videos/search?query=' + encodeURIComponent(query) + '&per_page=4&size=small', {
      headers: { 'Authorization': _pexelsKeys.pexels }
    }).then(r => { return r.json(); }).then(_handlePexelsResult).catch(_handlePexelsError);
  }
  _pexelsPending[cacheKey] = pendingPromise.finally(() => { delete _pexelsPending[cacheKey]; });
}
...
function _renderPexelsVideos(container, videos, elId) {
  ...
  videos.forEach((v, vi) => {
    ...
    const sdFile = v.video_files && v.video_files.find(f => { return f.quality === 'sd'; });
    const hdFile = v.video_files && v.video_files.find(f => { return f.quality === 'hd'; });
    const previewFile = sdFile || hdFile || (v.video_files && v.video_files[0]);
    const dlUrl = previewFile ? previewFile.link : v.url;
    const vidId = elId + '-' + vi;
    const alreadyDL = shared.pexelsDL.some(d => { return d.id === v.id; });

    const cardDiv = el('div', {
      id: vidId,
      style: 'flex-shrink:0;width:160px;border-radius:10px;overflow:hidden;border:1.5px solid ' + (alreadyDL ? 'var(--grn)' : 'var(--bdr)') + ';transition:all .2s'
    });
    ...
    if (thumb) {
      ...
      if (safeThumb && safeVUrl) {
        const link = el('a', { href: safeVUrl, target: '_blank', rel: 'noopener', style: 'display:block;position:relative' });
        const img = el('img', { style: 'width:160px;height:90px;object-fit:cover;display:block' });
        img.src = safeThumb;
        const overlay = el('div', { style: 'position:absolute;top:50%;left:50%;transform:translate(-50%,-50%);width:32px;height:32px;background:rgba(0,0,0,.6);border-radius:50%;display:flex;align-items:center;justify-content:center' });
        overlay.appendChild(el('span', { style: 'color:#fff;font-size:14px;margin-left:2px', textContent: '\u25B6' }));
    ...
    infoDiv.appendChild(el('div', { style: 'font-size:11px;color:var(--t3);white-space:nowrap;overflow:hidden;text-overflow:ellipsis;margin-bottom:8px', textContent: '\uD83D\uDCF7 ' + (v.user ? v.user.name : '') }));
    ...
    // 미리보기 버튼
    const previewBtn = el('button', { className: 'btn bs flex-1 t-2xs', style: 'padding:6px 0', textContent: '\uD83D\uDC41 미리보기' });
    previewBtn.addEventListener('click', e => {
      e.stopPropagation();
      if (!safeDlUrl) { toast('허용되지 않은 미디어 URL입니다.', 'err'); return; }
...
function _pexelsPreview(videoUrl) {
  ...
  if (!safe) { toast('잘못된 미리보기 URL입니다', 'err'); return; }
  const old = document.getElementById('pexelsModal');
  if (old) old.remove();
  const modal = el('div', { id: 'pexelsModal', style: 'position:fixed;top:0;left:0;width:100%;height:100%;background:rgba(0,0,0,.85);z-index:9999;display:flex;align-items:center;justify-content:center;cursor:pointer' });
  const wrap = el('div', { style: 'position:relative;max-width:80vw;max-height:80vh' });
  wrap.addEventListener('click', e => { e.stopPropagation(); });
  const video = el('video', { style: 'max-width:80vw;max-height:80vh;border-radius:12px;box-shadow:0 20px 60px rgba(0,0,0,.5)' });
  video.src = safe;
  video.controls = true;
  video.autoplay = true;
  const closeBtn = el('button', { style: 'position:absolute;top:-16px;right:-16px;width:36px;height:36px;border-radius:50%;background:var(--bg2);border:1px solid var(--bdr);color:var(--t1);font-size:18px;cursor:pointer;display:flex;align-items:center;justify-content:center', textContent: '\u2715' });
...
function _pexelsAddDL(id, url, author, vidId) {
  if (!url) { toast('허용되지 않은 다운로드 URL입니다.', 'err'); return; }
  
  const exists = shared.pexelsDL.some(d => { return d.id === id; });
  if (exists) {
    shared.pexelsDL = shared.pexelsDL.filter(d => { return d.id !== id; });
    ...
    if (btn) { btn.className = 'btn bs'; btn.textContent = '\u2B07 다운로드'; }
    toast('다운로드 목록에서 제거됨');
  } else {
    ...
    const btn = document.getElementById('dlbtn-' + vidId);
    if (btn) { btn.className = 'btn bp'; btn.textContent = '\u2713 추가됨'; }
...
function rAllEK() {
  ...
  if (!results.length) {
    ...
    root.appendChild(el('div', { className: 'cd', textContent: '풋티지 브리프를 표시할 스크립트가 없습니다.' }));
    return;
  }
  if (!results.every(isValidResult)) {
    _ekPage = 0;
    root.appendChild(el('h2', { className: 'pt', textContent: '풋티지 브리프' }));
    root.appendChild(el('div', { className: 'cd', textContent: '스크립트 데이터가 손상되었습니다. 이전 단계부터 다시 진행해주세요.' }));
    return;
  }
  ...
  root.appendChild(el('h2', { className: 'pt', textContent: '풋티지 브리프' }));
  root.appendChild(el('p', { className: 'pd', textContent: results.length + '개 스크립트 · ' + (page + 1) + '/' + results.length + ' 페이지' + (hasPexels ? ' · 키워드 클릭 시 Pexels 무료 영상 표시' : '') }));
  root.appendChild(_buildEkTabs(results, page));

  const pageHeader = el('div', { className: 'fx-row mb-16' });
  pageHeader.appendChild(el('span', { style: 'font-size:12px;font-weight:600;color:' + typeColor + ';background:' + typeColor + '12;padding:3px 8px;border-radius:4px', textContent: typeLabel }));
  pageHeader.appendChild(el('span', { className: 't-title', textContent: r.script.title || '' }));
  pageHeader.appendChild(el('span', { className: 'bdg bg2 t-3xs ml-auto', textContent: scenes.length + '장면' }));
  root.appendChild(pageHeader);

  if (scenes.length) scenes.forEach((s, i) => { root.appendChild(_buildSceneCard(s, i, page, hasPexels)); });
  else root.appendChild(el('div', { className: 'cd empty-msg', textContent: '풋티지 없음' }));

  // ── Storyblocks 키워드 모아보기 (전체 한글 번역) ──
  if (scenes.length) {
    const sbSection = el('div', { style: 'margin-top:24px;padding:20px;background:var(--bg);border:1.5px solid var(--bdr);border-radius:var(--r)' });
    sbSection.appendChild(el('div', { style: 'font-size:14px;font-weight:700;color:var(--t1);margin-bottom:4px', textContent: '🎬 Storyblocks 영상 소스' }));
    sbSection.appendChild(el('div', { style: 'font-size:12px;color:var(--t3);margin-bottom:14px', textContent: '키워드를 클릭하면 Storyblocks에서 유료 영상을 검색합니다.' }));

    const sbGrid = el('div', { id: 'sb-grid-' + page, style: 'display:flex;flex-direction:column;gap:10px' });

    // 모든 영어 키워드 수집
    const allKws = [];
    scenes.forEach(s => {
      if (s.mainEn) allKws.push(s.mainEn);
      (s.altEn || []).forEach(a => { if (a && a.trim()) allKws.push(a.trim()); });
    });
    const uniqueKws = [...new Set(allKws)];

    // 기존 ko 필드에서 이미 있는 번역 매핑
    const transMap = {};
    scenes.forEach(s => { if (s.mainEn && s.ko) transMap[s.mainEn.toLowerCase()] = s.ko; });

    // 키워드 블록 생성 함수
    function makeKwBlock(enText, koText) {
      const safe = safeUrl(sbUrl(enText), STORYBLOCKS_HOSTS);
      if (!safe) return null;
      const block = el('a', { href: safe, target: '_blank', rel: 'noopener', style: 'text-decoration:none;display:inline-flex;align-items:center;gap:6px;padding:5px 12px;background:var(--bg2);border-radius:12px;border:1px solid var(--bdr);transition:border-color .15s' });
      block.appendChild(el('span', { style: 'font-size:12px;font-weight:600;color:var(--acc)', textContent: enText }));
      if (koText) {
        block.appendChild(el('span', { style: 'font-size:12px;color:var(--t2)', textContent: koText }));
      }
      block.appendChild(el('span', { style: 'font-size:10px;color:var(--t4)', textContent: '↗' }));
      block.addEventListener('mouseenter', () => { block.style.borderColor = 'var(--acc)'; });
      block.addEventListener('mouseleave', () => { block.style.borderColor = 'var(--bdr)'; });
      return block;
    }
    ...
    function renderSbRows() {
      ...
      scenes.forEach((s, i) => {
        ...
        row.appendChild(el('span', { style: 'font-size:11px;font-weight:700;color:' + color + ';background:' + color + '15;padding:2px 8px;border-radius:4px;flex-shrink:0;min-width:70px', textContent: (i + 1) + '. ' + (s.label || '') }));
    ...
    if (untranslated.length > 0) {
      const transNote = el('div', { id: 'sb-trans-note-' + page, style: 'margin-top:8px;font-size:11px;color:var(--t4)', textContent: '⏳ ' + untranslated.length + '개 키워드 번역 중...' });
      sbSection.appendChild(transNote);

      // 비동기 번역 호출
      _batchTranslate(untranslated).then(results => {
        Object.keys(results).forEach(k => { transMap[k.toLowerCase()] = results[k]; });
        renderSbRows();
  ...
  if (page > 0) {
    ...
    prevBtn.addEventListener('click', () => { _ekGoPage(page - 1); });
    navRow.appendChild(prevBtn);
  } else navRow.appendChild(el('div'));
  if (page < results.length - 1) {
    const nextBtn = el('button', { className: 'btn bp', textContent: '다음 스크립트 →' });
    nextBtn.addEventListener('click', () => { _ekGoPage(page + 1); });
    navRow.appendChild(nextBtn);
  } else {
    const goBtn = el('button', { className: 'btn bp btn-lg', id: 's8b', textContent: '음성 생성 →' });
  ...
  try {
    while (true) {
      ...
      if (!results.length || !results.every(isValidResult)) {
        toast('풋티지 브리프를 만들 스크립트 데이터가 손상되었습니다. 이전 단계로 돌아갑니다.', 'err');
        sPrev();
        return;
      }
      const idx = results.findIndex(r => { return !r.ekw; });
      if (idx === -1) { _ekPage = 0; rAllEK(); return; }
      ...
      const root = $('p8');
      root.textContent = '';
      const txNotice = el('div', { style: 'margin-bottom:12px;padding:10px 14px;background:rgba(6,182,212,.06);border:1px solid rgba(6,182,212,.12);border-radius:var(--r);font-size:11px;color:var(--t3);display:flex;align-items:center;gap:8px' });
      txNotice.appendChild(el('span', { textContent: '🔒' }));
      txNotice.appendChild(el('span', { textContent: '대본 텍스트가 AI 서비스에 전송되어 장면 키워드를 추출합니다. 이후 키워드가 Pexels에 전송되어 영상을 검색합니다.' }));
      root.appendChild(txNotice);
      const loadDiv = el('div', { id: 'ekwLoad' });
      root.appendChild(loadDiv);
      const prog = createProgress('ekwLoad', '풋티지 브리프 (' + (idx + 1) + '/' + total + ')', stepLabels, 15 * total);
      for (let i = 0; i < idx; i++) prog.nextStep();

      try {
        const ekw = await withTimeout((signal) => Api.extractKw(current.script.content, { signal: mergeAbortSignals(signal, _ekLeaveAC.signal) }), 300000, '풋티지 브리프 생성 시간이 초과되었습니다.');
        if (runId !== _ekRunId) return;
...
async function _batchTranslate(keywords) {
  const prompt = '다음 영어 키워드를 한국어로 짧게 번역하세요. 각 줄에 "영어=한국어" 형식으로, 설명 없이 번역만 출력하세요.\n\n' + keywords.join('\n');
  let text = '';
  try {
  ...
  text.split('\n').forEach(line => {
    ...
    if (sep > 0) {
      const en = line.substring(0, sep).trim().replace(/^\d+[\.\)]\s*/, '');
...
registerStep(8, () => {
  ...
  if (results.every(r => Object.prototype.hasOwnProperty.call(r, 'ekw') && r.ekw !== null)) { _ekPage = 0; rAllEK(); return; }
  processAllFootage();
});
