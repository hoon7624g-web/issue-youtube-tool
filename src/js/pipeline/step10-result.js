...
function isVoiceFailed(result) {
  return !!(
    result &&
    result.voiceResult &&
    !hasVoiceAsset(result) &&
    (result.voiceResult.provider === 'failed' || result.voiceResult.error)
  );
}
...
registerStep(10, () => {
  ...
  if (!results.length) {
    ...
    root.appendChild(
      el('div', {
        className: 'cd',
        textContent: '결과 스크립트가 없습니다. 이전 단계부터 다시 진행해주세요.',
      })
    );
    return;
  }
  ...
  headerLeft.appendChild(
    el('div', {
      style:
        'width:52px;height:52px;border-radius:14px;background:linear-gradient(135deg,var(--grn),var(--grn2));display:flex;align-items:center;justify-content:center;font-size:24px;color:#fff;box-shadow:0 4px 16px rgba(5,150,105,.3)',
  ...
  headerText.appendChild(
    el('h2', { className: 'pt', style: 'margin:0', textContent: '제작 완료' })
  );
  headerText.appendChild(
    el('p', {
      style: 'font-size:13px;color:var(--t3);margin:4px 0 0',
      textContent:
        totalScripts +
        '개 스크립트 · ' +
        new Date().toLocaleDateString('ko', { month: 'long', day: 'numeric', weekday: 'short' }),
    })
  );
  ...
  const dlPkgBtn = el('button', {
    className: 'btn btn-download',
    textContent: '\uD83D\uDCE6 패키지 다운로드',
  });
  dlPkgBtn.addEventListener('click', () => {
    runAction('downloadPkg');
  });
  ...
  // 패키지 내용물 안내
  const pkgDesc = el('div', {
    style:
      'margin-bottom:20px;padding:10px 16px;background:var(--bg);border:1px solid var(--bdr);border-radius:var(--r);font-size:12px;color:var(--t3);display:flex;align-items:center;gap:8px',
  });
  ...
  if (voiceFailedCount > 0) {
    const warnBanner = el('div', {
      style:
        'margin-bottom:20px;padding:12px 16px;background:var(--red-bg);border:1px solid rgba(201,42,42,.15);border-radius:var(--r);font-size:13px;color:var(--red);display:flex;align-items:center;gap:10px',
    });
    ...
    warnText.appendChild(
      el('div', {
        ...
        textContent: '음성 생성 실패 ' + voiceFailedCount + '건',
      })
    );
    warnText.appendChild(
      el('div', {
        ...
        textContent:
          '해당 스크립트는 ZIP 패키지에 음성 파일이 포함되지 않습니다. 이전 단계에서 재시도할 수 있습니다.',
      })
    );
  ...
  const statsGrid = el('div', {
    style:
      'display:grid;grid-template-columns:repeat(auto-fit,minmax(120px,1fr));gap:14px;margin-bottom:24px',
  });
  const statItems = [
  ...
  statItems.forEach((st, si) => {
    const box = el('div', {
      className: 'cd stagger-' + (si + 1),
      style: 'padding:20px;text-align:center;margin-bottom:0',
    });
    box.appendChild(
      el('div', { className: 'stat-hero', style: 'color:' + st.color, textContent: st.val })
    );
  ...
  origInfo.appendChild(
    el('div', {
      style:
        'font-size:17px;font-weight:800;line-height:1.4;letter-spacing:-.2px;margin-bottom:6px',
      textContent: v.title || '',
    })
  );
  origInfo.appendChild(
    el('div', {
      ...
      textContent:
        (v.ch || '') + ' · \u25B6 ' + fmt(v.views || 0) + (v.subs ? ' · 구독 ' + fmt(v.subs) : ''),
    })
  );
  ...
  if (results.length > 1) {
    ...
    results.forEach((r, i) => {
      ...
      btn.appendChild(
        el('span', { className: 't-2xs-t3', textContent: ' ' + r.script.content.length + '자' })
      );
  ...
  (() => {
    ...
    const card = el('div', {
      className: 'cd',
      style: 'margin-bottom:14px;border-left:4px solid ' + typeColor,
    });
    ...
    copyBtn.addEventListener('click', () => {
      navigator.clipboard.writeText(r.script.content).then(() => {
        toast('복사됨');
      });
    });
    ...
    badgeRow.appendChild(
      el('span', { className: 'bdg bgy', textContent: r.script.content.length + '자' })
    );
    badgeRow.appendChild(
      el('span', { className: 'bdg bgy', textContent: '팩트체크 ' + fcs.length + '건' })
    );
    badgeRow.appendChild(
      el('span', { className: 'bdg bgy', textContent: '풋티지 ' + ekw.length + '장면' })
    );
    if (vr.url) {
      badgeRow.appendChild(
        el('span', {
          className: 'bdg bg2',
          textContent: '\uD83D\uDD0A ' + mn + ':' + String(sc).padStart(2, '0'),
        })
      );
      ...
      badgeRow.appendChild(
        el('span', {
          style:
            'font-size:11px;padding:2px 8px;border-radius:4px;background:var(--red-bg);color:var(--red);font-weight:600',
          textContent: '⚠ 음성 실패',
        })
      );
    }
    ...
    const scriptDiv = el('div', {
      className: 'out',
      style:
        'max-height:300px;overflow-y:auto;padding:12px;background:var(--bg);border-radius:var(--r);font-size:13px;line-height:1.8;white-space:pre-wrap',
    });
    ...
    if (vr.url) {
      ...
      if (safeAudioUrl) {
        ...
        const playBtn = el('button', {
          className: 'btn bs',
          style: 'width:32px;height:32px;border-radius:50%;padding:0;font-size:14px',
          textContent: '\u25B6',
        });
        ...
        const barOuter = el('div', {
          style: 'flex:1;height:4px;background:var(--bg3);border-radius:2px',
        });
      ...
      const errBox = el('div', {
        style:
          'margin-top:12px;padding:10px 14px;background:var(--red-bg);border-radius:var(--r);border:1px solid rgba(201,42,42,.15);font-size:12px;color:var(--red)',
        textContent:
          '음성 생성 실패: ' +
          (vr.error || '알 수 없는 오류') +
          ' — 이전 단계에서 재시도할 수 있습니다.',
  ...
  if (results.length > 1) {
    ...
    navRow.appendChild(
      el('span', { className: 't-xs-t3', textContent: page + 1 + ' / ' + results.length })
    );
  ...
  pkgHeader.appendChild(
    el('div', { className: 'st', style: 'margin:0', textContent: '프로젝트 패키지 구성' })
  );
  const toggleHint = el('span', {
    style: 'font-size:11px;color:var(--t3);margin-left:auto',
    textContent: '클릭하여 접기',
  });
  ...
  results.forEach((r) => {
    const prefix =
      r.script.type === 'longform' ? 'longform/' : 'short_' + ((r.script.idx || 0) + 1) + '/';
    ...
    folderDiv.appendChild(
      el('div', {
        style: 'font-size:12px;font-weight:600;color:var(--t2);margin-bottom:6px',
        textContent: '\uD83D\uDCC1 ' + prefix + ' (' + folderTypeLabel + ')',
      })
    );
    const fileList = el('div', {
      style: 'display:flex;flex-direction:column;gap:4px;padding-left:16px',
    });
    fileList.appendChild(
      el('div', {
        className: 't-xs-t3',
        textContent:
          '\uD83D\uDCC4 script.txt \u2014 ' + fmtB(new Blob([r.script.content || '']).size),
      })
    );
    if ((r.fcs || []).length) {
      fileList.appendChild(
        el('div', {
          className: 't-xs-t3',
          textContent: '\uD83D\uDCCA factcheck.json \u2014 ' + (r.fcs || []).length + '건',
        })
      );
    }
    if ((r.ekw || []).length) {
      fileList.appendChild(
        el('div', {
          className: 't-xs-t3',
          textContent: '\uD83C\uDFAC footage-brief.txt \u2014 ' + (r.ekw || []).length + '장면',
        })
      );
    }
    if (Array.isArray(r.voiceResult?.parts) && r.voiceResult.parts.length > 1) {
      r.voiceResult.parts.forEach((part, idx) => {
        fileList.appendChild(
          el('div', {
            className: 't-xs-t3',
            textContent:
              '\uD83D\uDD0A voice/voice_part_' + (idx + 1) + '.mp3 \u2014 ' + fmtB(part.size || 0),
          })
        );
      });
      ...
      fileList.appendChild(
        el('div', {
          className: 't-xs-t3',
          textContent: '\uD83D\uDD0A voice.mp3 \u2014 ' + fmtB(r.voiceResult.blob.size),
        })
      );
      ...
      fileList.appendChild(
        el('div', {
          className: 't-xs-t3',
          textContent: '\uD83D\uDD0A voice.mp3 \u2014 ' + fmtB(r.voiceResult.parts[0].size || 0),
        })
      );
    }
  ...
  const analysisLine = el('div', {
    style: 'font-size:12px;color:var(--t3);padding-top:8px;border-top:1px solid var(--bdr)',
    textContent: '\uD83D\uDCCA analysis.json \u2014 영상 분석 요약',
  });
  pkgFiles.appendChild(analysisLine);
  const metaLine = el('div', {
    style: 'font-size:12px;color:var(--t3)',
    textContent: '\uD83D\uDCCB project-info.json \u2014 메타데이터',
  });
  ...
  try {
    ...
    results.forEach((r) => {
      ...
      if (Array.isArray(r.voiceResult?.parts) && r.voiceResult.parts.length > 1) {
        ...
        fileList.push(prefix + '/voice.mp3');
    });
    fileList.push('analysis.json');
    fileList.push('project-info.json');
    if (pxDL.length)
      pxDL.forEach((p, i) => {
        fileList.push('footage/footage_' + (i + 1) + '.mp4');
      });
    ...
    const zip = new JSZip();
    const folderName = (v.title || '유튜브도사_' + new Date().toISOString().slice(0, 10)).replace(
      /[<>:"/\\|?*]/g,
      '_'
    );
    ...
    results.forEach((r) => {
      ...
      if (Array.isArray(r.voiceResult?.parts) && r.voiceResult.parts.length > 1) {
        ...
        folder.file('voice.mp3', r.voiceResult.parts[0]);
    });
    ...
    if (pxDL.length) {
      ...
      const progWrap = el('div', {
        id: '_pxProg',
        style:
          'position:fixed;bottom:80px;right:24px;background:var(--white);border:1px solid var(--bdr);border-radius:var(--r2);padding:14px 18px;box-shadow:var(--shadow-md);z-index:9999;min-width:240px',
      });
      progWrap.appendChild(
        el('div', {
          style: 'font-size:12px;font-weight:600;color:var(--t1);margin-bottom:8px',
          textContent: '\uD83C\uDFAC 풋티지 다운로드',
        })
      );
      ...
      const progFill = el('div', {
        style: 'height:100%;background:var(--acc);border-radius:3px;width:0%;transition:width .3s',
      });
      progBar.appendChild(progFill);
      progWrap.appendChild(progBar);
      const progText = el('div', {
        style: 'font-size:11px;color:var(--t3)',
        textContent: '0/' + pxDL.length + ' 다운로드 중...',
      });
      ...
      function updatePxProgress() {
        ...
        progText.textContent =
          ...
          pxDL.length +
          ' 완료' +
          (pxFailed.length ? ' (' + pxFailed.length + '개 실패)' : '');
      }
      ...
      async function fetchFootageBlob(url, timeoutMs) {
        ...
        try {
          ...
          try {
            ...
            if (
              !PEXELS_HOSTS.some((host) => {
                return finalUrl.hostname === host || finalUrl.hostname.endsWith('.' + host);
              })
            ) {
        ...
        async function worker() {
          while (idx < items.length) {
            if (totalDownloadedBytes >= MAX_TOTAL_SIZE) {
              ...
              if (remaining > 0)
                toast('풋티지 총 용량 제한(200MB) 초과 — 나머지 ' + remaining + '개 건너뜀', 'err');
              break;
            }
            const i = idx++;
            const p = items[i];
            ...
            try {
              ...
              const safeAuthor =
                ((p.author || 'unknown') + '').replace(/[^a-zA-Z0-9]/g, '_').substring(0, 20) ||
                'unknown';
      ...
      downloadWithLimit(pxDL, MAX_CONCURRENT)
        .then(async () => {
