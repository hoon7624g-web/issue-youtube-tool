...
sOn(K.NAV_STEP, (step) => {
  if (step !== 9) {
    ...
    try {
      _voiceLeaveAC.abort(new Error('step-leave'));
    } catch (e) {}
...
function invalidateVoiceResultsForCurrentConfig(notify) {
  ...
  if (changed) {
    ...
    if (notify)
      toast('음성/배속 설정이 바뀌어 기존 음성을 초기화했습니다. 다시 생성해주세요.', 'err');
  }
  return changed;
}
...
function _buildVoiceCard(v, selId) {
  ...
  const card = el('div', {
    className: 'tag' + (on ? ' on' : ''),
    style:
      'padding:10px 18px;font-size:13px;display:inline-flex;flex-direction:column;align-items:center;gap:4px;min-width:72px;cursor:pointer' +
      (on && isEl ? ';border-color:#8B5CF6' : ''),
  });
  card.dataset.vid = v.id;
  const dot = el('div', {
    style:
      'width:36px;height:36px;border-radius:50%;background:' +
      (on ? (isEl ? 'rgba(139,92,246,.15)' : 'var(--acc-bg2)') : 'var(--bg2)') +
      ';display:flex;align-items:center;justify-content:center;font-size:' +
      (isEl ? '10' : '12') +
      'px;font-weight:700;color:' +
      (on ? (isEl ? '#8B5CF6' : 'var(--acc)') : 'var(--t3)'),
...
function renderVoicePreview(vid) {
  ...
  if (!v && vid === 'el-custom' && S.voice.customElVoiceId) {
    v = {
      ...
      name: '커스텀 보이스',
      gender: '',
      desc: S.voice.customElVoiceId.substring(0, 12) + '...',
      provider: 'elevenlabs',
      elId: S.voice.customElVoiceId,
    };
  }
  if (!v) return;
  const isEl = v.provider === 'elevenlabs';
  ...
  const wrap = el('div', {
    style:
      'display:flex;align-items:center;gap:16px;padding:16px;background:var(--bg);border:1px solid ' +
      (isEl ? 'rgba(139,92,246,.3)' : 'var(--bdr)') +
  ...
  const icon = el('div', {
    style:
      'width:44px;height:44px;border-radius:50%;background:' +
      (isEl ? 'rgba(139,92,246,.15)' : 'var(--acc-bg2)') +
      ';display:flex;align-items:center;justify-content:center;flex-shrink:0',
  });
  icon.appendChild(
    el('span', {
      style: 'font-size:' + (isEl ? '10' : '12') + 'px;font-weight:700;color:' + accentColor,
      textContent: isEl ? 'PRO' : 'AI',
    })
  );
  ...
  nameRow.appendChild(
    el('span', {
      style: 'font-size:12px;color:var(--t3);font-weight:400;margin-left:8px',
      textContent: (v.gender === '남' ? '남성' : '여성') + ' · ' + (v.desc || ''),
    })
  );
  nameRow.appendChild(
    el('span', {
      className: 'bdg',
      style:
        'font-size:9px;margin-left:6px;background:' +
        (isEl ? 'rgba(139,92,246,.1)' : 'rgba(34,197,94,.1)') +
        ';color:' +
        (isEl ? '#8B5CF6' : '#22C55E'),
  ...
  const wave = el('div', {
    id: 'vpWave',
    style: 'flex:1;height:24px;display:flex;align-items:center;gap:2px',
  });
  for (let i = 0; i < 30; i++) {
    ...
    wave.appendChild(
      el('div', {
        style:
          'width:3px;height:' +
          h +
          'px;background:var(--bg3);border-radius:1px;transition:background .2s',
      })
    );
  }
  ctrlRow.appendChild(wave);
  ctrlRow.appendChild(
    el('span', { id: 'vpTime', className: 't-mono-xs', textContent: '미리듣기' })
  );
...
function _buildVoiceResultCard(r, page, spd) {
  ...
  hdr.appendChild(
    el('span', {
      style: 'font-family:var(--mono);font-size:12px;color:var(--t3);margin-left:auto',
      textContent:
        mn + ':' + String(sc).padStart(2, '0') + (spd > 1 ? ' (' + spd.toFixed(1) + 'x)' : ''),
    })
  );
  ...
  const previewDiv = el('div', {
    style:
      'max-height:120px;overflow-y:auto;padding:12px;background:var(--bg);border-radius:var(--r);font-size:13px;line-height:1.8;margin-bottom:14px;color:var(--t2)',
  });
  ...
  if (vr.url) {
    ...
    if (safeAudioUrl) {
      ...
      const playBtn = el('button', {
        className: 'btn bs',
        style: 'width:36px;height:36px;border-radius:50%;padding:0;font-size:14px',
        textContent: '\u25B6',
      });
      ...
      const barOuter = el('div', {
        style: 'flex:1;height:4px;background:var(--bg3);border-radius:2px;position:relative',
      });
    ...
    const errBox = el('div', {
      style:
        'padding:14px;background:var(--red-bg);border-radius:var(--r);border:1px solid rgba(201,42,42,.15)',
    });
    errBox.appendChild(
      el('div', {
        style: 'font-size:13px;color:var(--red);margin-bottom:8px',
        textContent: '음성 생성 실패: ' + (vr.error || '알 수 없는 오류'),
      })
    );
    const retryBtn = el('button', {
      className: 'btn bp',
      style: 'font-size:12px;padding:6px 14px',
      textContent: '이 스크립트만 재시도',
    });
...
function _buildFootageBrief(ekw, typeColor) {
  const footCard = el('div', {
    className: 'cd',
    style: 'margin-top:14px;border-left:4px solid ' + typeColor,
  });

  const footHdr = el('div', { className: 'fx-row mb-14' });
  footHdr.appendChild(el('span', { className: 't-16', textContent: '\uD83C\uDFAC' }));
  footHdr.appendChild(
    el('div', { style: 'font-size:14px;font-weight:700', textContent: '풋티지 브리프' })
  );
  footHdr.appendChild(
    el('span', { className: 'bdg bg2 t-3xs ml-auto', textContent: ekw.length + '장면' })
  );
  ...
  ekw.forEach((s, si) => {
    ...
    const scene = el('div', {
      style:
        'display:flex;gap:10px;padding:10px 12px;margin-bottom:6px;background:var(--bg);border-radius:var(--r)',
    });
    ...
    scene.appendChild(
      el('div', {
        style:
          ...
          color +
          '15;display:flex;align-items:center;justify-content:center;flex-shrink:0;font-size:11px;font-weight:700;color:' +
          color,
        textContent: String(si + 1),
      })
    );
    ...
    labelRow.appendChild(
      el('span', {
        style: 'font-size:11px;font-weight:600;color:' + color,
        textContent: s.label || '',
      })
    );
    ...
    info.appendChild(
      el('div', {
        style: 'font-size:12px;color:var(--t2);line-height:1.5;margin-bottom:4px',
        textContent: '\u201C' + (s.text || '') + '\u201D',
      })
    );
    ...
    if (s.mainEn)
      kwRow.appendChild(
        el('span', {
          className: 'tag on t-3xs',
          style: 'padding:2px 8px',
          textContent: '\uD83D\uDD0D ' + s.mainEn,
        })
      );
    (s.altEn || []).forEach((alt) => {
      kwRow.appendChild(
    ...
    if (s.ko)
      kwRow.appendChild(
        el('span', {
          className: 'tag',
          style: 'font-size:10px;padding:2px 8px;background:var(--bg2);color:var(--t2)',
          textContent: '\uD83D\uDD0D ' + s.ko,
        })
      );
...
function rAllVoice() {
  ...
  if (!results.length) {
    ...
    root.appendChild(
      el('div', {
        className: 'cd',
        textContent: '표시할 음성 결과가 없습니다. 이전 단계부터 다시 진행해주세요.',
      })
    );
    return;
  }
  if (!results.every(isValidResult)) {
    ...
    root.appendChild(
      el('div', {
        className: 'cd',
        textContent: '음성 결과 데이터가 손상되었습니다. 이전 단계부터 다시 진행해주세요.',
      })
    );
    return;
  }
  ...
  const headerText =
    failedCount > 0 ? '음성 생성 완료 (' + failedCount + '건 실패)' : '음성 생성 완료';
  ...
  if (failedCount > 0) {
    root.appendChild(
      el('div', {
        style:
          'padding:10px 14px;background:var(--red-bg);border:1px solid rgba(201,42,42,.15);border-radius:var(--r);margin-bottom:16px;font-size:13px;color:var(--red)',
        textContent:
          '⚠ ' +
          failedCount +
          '건의 음성 생성이 실패했습니다. 각 카드에서 개별 재시도할 수 있습니다.',
      })
    );
  }
  root.appendChild(
    el('p', {
      ...
      textContent:
        ...
        results.length +
        ' 페이지' +
        (spd > 1 ? ' · 배속: ' + spd.toFixed(1) + 'x' : ''),
    })
  );
  root.appendChild(
    el('div', {
      style: 'font-size:10px;color:var(--t4);margin:-16px 0 16px',
      textContent: '대본 텍스트가 TTS 서비스에 전송되어 음성이 생성되었습니다',
    })
  );
    ...
    while (true) {
      ...
      const prog = createProgress(
        ...
        '음성 생성 (' +
          ...
          total +
          ') · ' +
          (S.voice.voiceSpeed || 1.0).toFixed(1) +
          'x',
        stepLabels,
        25 * total
      );
      ...
      try {
        ...
        const result = await withTimeout(
          (signal) =>
            Api.genVoice(script, S.voice.selVoice, {
              signal: mergeAbortSignals(signal, _voiceLeaveAC.signal),
            }),
          dynamicTimeout,
          '음성 생성 시간이 초과되었습니다 (' + Math.round(dynamicTimeout / 1000) + '초).'
        );
        ...
        const newResults = S.script.results.slice();
        newResults[idx] = Object.assign({}, newResults[idx], {
          voiceResult: {
            dur: 0,
            provider: 'failed',
            error: errMsg,
            speed: S.voice.voiceSpeed || 1.0,
            voiceId: S.voice.selVoice || 'vc4',
            _voiceSig: getVoiceConfigSig(),
          },
        });
        sSet({ [K.SCRIPT_RESULTS]: newResults });
...
registerStep(9, () => {
  ...
  if (results.every((r) => isVoiceTerminal(r.voiceResult))) {
    _voicePage = 0;
    rAllVoice();
    return;
  }
  ...
  const googleVoices = voices.filter((v) => {
    return v.provider === 'google';
  });
  const elVoices = voices.filter((v) => {
    return v.provider === 'elevenlabs';
  });
  ...
  root.appendChild(
    el('p', {
      className: 'pd',
      textContent: 'AI 음성을 선택하면 ' + results.length + '개 스크립트의 음성을 순차 생성합니다',
    })
  );
  ...
  gHeader.appendChild(
    el('span', {
      className: 'bdg',
      style: 'font-size:10px;background:rgba(34,197,94,.1);color:#22C55E',
      textContent: '무료',
    })
  );
  ...
  googleVoices.forEach((v) => {
    gRow.appendChild(_buildVoiceCard(v, selId));
  });
  voiceCard.appendChild(gRow);
  const elSection = el('div', {
    style: 'border-top:1px solid var(--bdr);padding-top:16px;margin-top:4px',
  });
  ...
  eHeader.appendChild(
    el('span', {
      className: 'bdg',
      style: 'font-size:10px;background:rgba(139,92,246,.1);color:#8B5CF6',
      textContent: 'ElevenLabs',
    })
  );
  elSection.appendChild(eHeader);
  const eRow = el('div', { className: 'fx-wrap-8' });
  elVoices.forEach((v) => {
    eRow.appendChild(_buildVoiceCard(v, selId));
  });
  elSection.appendChild(eRow);

  // 커스텀 ElevenLabs 보이스 ID 입력
  const customElRow = el('div', {
    style:
      'margin-top:12px;padding:12px;background:var(--bg);border-radius:var(--r);border:1px solid var(--bdr)',
  });
  customElRow.appendChild(
    el('div', {
      style: 'font-size:12px;font-weight:600;color:var(--t2);margin-bottom:8px',
      textContent: '🎙️ 커스텀 보이스 ID (ElevenLabs Voice Library에서 복사)',
    })
  );
  const customElInput = el('input', {
    className: 'inp',
    id: 'customElVoiceId',
  ...
  const customElHelp = el('a', {
    style: 'font-size:11px;color:var(--blu);cursor:pointer;margin-left:8px',
    textContent: 'Voice Library에서 찾기 ↗',
  });
  customElHelp.addEventListener('click', (e) => {
    e.preventDefault();
    window.open('https://elevenlabs.io/voice-library?language=ko&sort=trending', '_blank');
  });
  const customElActions = el('div', {
  ...
  if (S.voice.elVoiceId) {
    const elBadge = el('div', {
      style: 'border-top:1px solid var(--bdr);padding-top:16px;margin-top:16px',
    });
    elBadge.appendChild(
      el('span', {
        className: 'bdg',
        style: 'font-size:11px;background:rgba(139,92,246,.1);color:#8B5CF6',
        textContent: '🎤 내 목소리 등록됨',
      })
    );
  ...
  spdHeader.appendChild(
    el('span', {
      className: 'bdg',
      style: 'font-size:10px;background:rgba(6,182,212,.1);color:#0891B2',
      textContent: '숏폼 추천: 1.2~1.3x',
    })
  );
  ...
  const slider = el('input', {
    id: 'speedSlider',
    style: 'flex:1;accent-color:var(--acc);height:6px;cursor:pointer',
  });
  slider.type = 'range';
  slider.min = '1.0';
  slider.max = '1.5';
  slider.step = '0.1';
  slider.value = String(S.voice.voiceSpeed || 1.0);
  sliderRow.appendChild(slider);
  const speedVal = el('span', {
    id: 'speedValue',
    className: 'speed-display',
    textContent: (S.voice.voiceSpeed || 1.0).toFixed(1) + 'x',
  });
  sliderRow.appendChild(speedVal);
  speedCard.appendChild(sliderRow);
  const labelRow = el('div', {
    style:
      'display:flex;justify-content:space-between;font-size:11px;color:var(--t4);padding:0 2px',
  });
  ...
  const estChunks = results.reduce(
    (sum, r) => sum + Math.ceil((r.script.content || '').length / 1500),
    0
  );
  ...
  const estBox = el('div', {
    style:
      'margin-top:16px;padding:14px 18px;background:var(--bg);border:1px solid var(--bdr);border-radius:var(--r2);font-size:12px;color:var(--t2);line-height:1.8',
  });
  estBox.appendChild(
    el('div', {
      textContent:
  ...
  slider.addEventListener('input', () => {
    speedVal.textContent = Number(slider.value).toFixed(1) + 'x';
  });
  slider.addEventListener('change', () => {
    S.voice.voiceSpeed = Number(slider.value);
    renderVoicePreview(S.voice.selVoice || selId);
  });
  renderVoicePreview(S.voice.selVoice || selId);
  root.querySelectorAll('[data-vid]').forEach((card) => {
    card.addEventListener('click', () => {
      S.voice.selVoice = card.dataset.vid;
      root.querySelectorAll('[data-vid]').forEach((x) => x.classList.remove('on'));
      card.classList.add('on');
      renderVoicePreview(card.dataset.vid);
    });
  });
