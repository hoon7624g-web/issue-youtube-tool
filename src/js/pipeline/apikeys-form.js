...
export function isSessionOnlyMode() {
  return _sessionOnlyMode;
}
export function setSessionOnlyMode(v) {
  _sessionOnlyMode = v;
}
...
export async function saveApiKeys() {
  ...
  if (!ytOk) {
    toast('YouTube API 키를 입력해주세요', 'err');
    return false;
  }
  const llmOk =
  ...
  if (!result || !result.ok)
    throw new Error((result && result.error) || 'API 키 저장에 실패했습니다.');
  return result;
}
...
export function llmTabClick(selected) {
  ...
  Object.values(keyMap).forEach((id) => {
    const e = document.getElementById(id);
    if (e) {
      const wrap = e.closest('.key-input-wrap');
      if (wrap) wrap.style.display = id === keyMap[selected] ? 'flex' : 'none';
    }
  });
  const gf = document.getElementById('gaiStudioField');
  if (gf) {
    gf.style.opacity = selected === 'llmGemini' ? '0.5' : '1';
    gf.style.pointerEvents = selected === 'llmGemini' ? 'none' : 'auto';
  }
...
function _mkLabelDOM(mainText, badgeText, badgeColor) {
  const label = el('label', {
    style: 'font-weight:600;display:flex;align-items:center;gap:6px;flex-wrap:wrap',
  });
  label.appendChild(document.createTextNode(mainText));
  if (badgeText) {
    label.appendChild(
      el('span', {
        style: 'font-size:12px;color:' + (badgeColor || 'var(--red)') + ';font-weight:400',
...
function _createEyeToggle(inputId) {
  const btn = el('button', {
    className: 'btn bg',
    style:
      'padding:4px 8px;font-size:14px;flex-shrink:0;border-radius:6px;min-width:32px;line-height:1',
    textContent: '\uD83D\uDC41',
  });
  ...
  btn.addEventListener('click', () => {
    ...
    if (inp.type === 'password') {
      ...
      btn.textContent = '\uD83D\uDC41';
      btn.title = '키 보기 (5초 후 자동 숨김)';
      if (_autoHideTimers[inputId]) {
        clearTimeout(_autoHideTimers[inputId]);
        _autoHideTimers[inputId] = null;
      }
    }
  });
  return btn;
...
function _createInlineStatus(inputId) {
  return el('span', {
    id: inputId + '_status',
    style:
      'font-size:11px;font-weight:500;min-width:20px;flex-shrink:0;text-align:center;transition:all .2s',
  });
}
  ...
  const handler = () => {
    ...
    if (inputId === 'keyGaiStudio') {
      ...
      if (geminiTab && geminiTab.classList.contains('on')) {
        ...
        if (s) {
          s.textContent = '\u2191';
          s.title = 'Gemini 키가 자동으로 적용됩니다';
          s.style.color = 'var(--blu)';
        }
        return;
      }
    }
    ...
    if (!key) {
      _setInlineStatus(inputId, 'clear');
      return;
    }
    ...
    _validateTimers[inputId] = setTimeout(async () => {
      ...
      try {
        ...
        if (result.skip) {
          ...
          _setInlineStatus(inputId, 'warn', result.msg || '형식만 확인됨 (실제 호출 시 검증)');
        } else if (result.ok) {
          _setInlineStatus(inputId, 'ok', result.msg || '연결 성공');
        } else {
          _setInlineStatus(inputId, 'fail', result.msg || '연결 실패');
        }
      } catch (e) {
...
function _mkKeyField(mainLabel, badgeText, badgeColor, id, savedFlag, placeholder, noteText, opts) {
  ...
  const inp = el('input', {
    className: 'inp',
    id: id,
    style: 'font-family:var(--mono);font-size:13px;flex:1' + (opts.inputStyle || ''),
  });
  ...
  if (noteText)
    field.appendChild(
      el('p', {
        style: 'font-size:11px;color:var(--t4);margin-top:4px;line-height:1.5',
  ...
  if (savedFlag) {
    ...
    if (lbl)
      lbl.appendChild(
        el('span', {
          style:
            'margin-left:8px;font-size:10px;font-weight:600;padding:2px 6px;border-radius:4px;background:rgba(13,146,84,.1);color:var(--grn)',
          textContent: '\u2713 저장됨',
        })
      );
  }
...
function _mkSelect(id, options, selectedValue, style) {
  const sel = el('select', {
    className: 'inp',
    id: id,
    style: style || 'font-size:13px;padding:10px 14px',
  });
  options.forEach((o) => {
    const opt = el('option');
    opt.value = o.value;
...
function _mkStepHeader(step, color, title, subtitle) {
  ...
  row.appendChild(
    el('span', { style: 'font-size:13px;font-weight:700;color:' + color, textContent: step })
  );
  row.appendChild(
    el('span', { style: 'font-size:13px;font-weight:600;color:var(--t1)', textContent: title })
  );
...
function _mkLlmTab(id, label, isOn) {
  return el('div', {
    ...
    id: id,
    style: 'padding:10px 16px;font-size:13px;cursor:pointer;flex:1;text-align:center',
    textContent: label,
  });
}
...
export function buildApiKeyFormDOM(keys, btnLabel, container) {
  ...
  container.appendChild(
    el('p', {
      className: 'pd',
      textContent:
        '각 서비스의 API 키를 등록해주세요. 키는 이 컴퓨터에만 저장되며 서버로 전송되지 않습니다.',
    })
  );
  ...
  const guide = el('div', {
    style:
      'margin-bottom:16px;padding:12px 16px;background:rgba(37,99,235,.06);border:1px solid rgba(37,99,235,.15);border-radius:var(--r2);display:flex;align-items:center;gap:10px;flex-wrap:wrap',
  });
  guide.appendChild(el('span', { style: 'font-size:16px', textContent: '\uD83D\uDCD6' }));
  const guideText = el('div', {
    style: 'flex:1;font-size:13px;color:var(--t2);line-height:1.5;min-width:200px',
  });
  ...
  const guideLink = el('a', {
    id: 'apiGuideLink',
    style:
      'font-size:12px;color:var(--acc);white-space:nowrap;text-decoration:none;font-weight:700;background:var(--acc-bg);padding:4px 10px;border-radius:6px;border:1px solid var(--acc-ring);cursor:pointer',
    textContent: '\uD83D\uDCD6 발급 가이드 \u2197',
  });
  guideLink.href = '#';
  ...
  [
    ...
    const a = el('a', {
      style:
        ...
        lnk.color +
        ';white-space:nowrap;text-decoration:none;font-weight:600;cursor:pointer',
      textContent: lnk.text,
    });
    a.href = lnk.href;
    a.target = '_blank';
    a.rel = 'noopener';
  ...
  container.appendChild(
    _mkStepHeader('1단계', 'var(--acc)', '필수 설정', '— 이것만 있으면 바로 시작할 수 있습니다')
  );
  const reqCard = el('div', {
    className: 'cd',
    style: 'padding:24px;border-color:var(--acc-ring)',
  });
  reqCard.appendChild(
    _mkKeyField(
      'YouTube API Key',
      ...
      !!keys.youtube,
      'AIza...',
      'Google Cloud Console \u2192 YouTube Data API v3 활성화 \u2192 API 키'
    )
  );
  ...
  llmTabs.querySelectorAll('[id]').forEach((t) => {
    t.dataset.llm = t.id;
  });
  ...
  const mkLlmInput = (id, savedFlag, fallbackPh, show) => {
    const wrap = el('div', {
      className: 'key-input-wrap',
      style: 'display:' + (show ? 'flex' : 'none') + ';align-items:center;gap:6px',
    });
    const inp = el('input', {
      className: 'inp',
      id: id,
  ...
  llmField.appendChild(
    el('p', {
      style: 'font-size:11px;color:var(--t4);margin-top:4px;line-height:1.5',
      textContent:
        'Claude: console.anthropic.com | Gemini: aistudio.google.com | ChatGPT: platform.openai.com\n\uD83D\uDCA1 Gemini를 선택하면 같은 키로 영상 분석(Google AI)도 가능합니다.',
    })
  );
  ...
  const gaiField = _mkKeyField(
    'Google AI 키 (영상 분석)',
    '*필수',
    'var(--red)',
    'keyGaiStudio',
    isGemini ? false : !!keys.googleAiStudio,
    isGemini ? 'Gemini 키가 자동 적용됩니다' : 'aistudio.google.com에서 발급한 키 입력',
    'YouTube 영상을 AI로 직접 분석할 때 사용합니다. Gemini 키와 동일한 Google AI Studio 키입니다.',
    { fieldId: 'gaiStudioField', fieldStyle: isGemini ? 'opacity:0.5;pointer-events:none' : '' }
  );
  // P0-1: DOM으로 gaiNote 조립
  const gaiNote = el('div', {
    id: 'gaiStudioNote',
    style:
      'display:' +
      (isGemini ? 'block' : 'none') +
      ';margin-top:8px;padding:10px 14px;background:rgba(66,133,244,.06);border:1px solid rgba(66,133,244,.15);border-radius:var(--r2);font-size:12px;color:#4285F4;line-height:1.6',
  });
  gaiNote.appendChild(document.createTextNode('\uD83D\uDCA1 '));
  gaiNote.appendChild(
    el('strong', {
      textContent: 'AI 모델로 Gemini를 선택하면 같은 키가 영상 분석에도 자동 적용됩니다.',
    })
  );
  ...
  // 시작 가능 안내
  const startHint = el('div', {
    style:
      'margin:16px 0;padding:14px 18px;background:rgba(13,146,84,.06);border:1px solid rgba(13,146,84,.15);border-radius:var(--r2);display:flex;align-items:center;gap:10px',
  });
  startHint.appendChild(el('span', { style: 'font-size:18px', textContent: '\u2705' }));
  const startHintText = el('div');
  startHintText.appendChild(
    el('div', {
      style: 'font-size:13px;font-weight:600;color:var(--grn)',
      textContent: '위 필수 키만 입력하면 바로 시작할 수 있습니다',
    })
  );
  startHintText.appendChild(
    el('div', {
      style: 'font-size:11px;color:var(--t3);margin-top:2px',
      textContent:
        '음성 생성, 풋티지, 팩트체크 강화 등은 아래 선택 설정에서 나중에 추가할 수 있어요.',
    })
  );
  ...
  // ═══ 2단계: 선택 키 ═══
  container.appendChild(
    _mkStepHeader('2단계', 'var(--t3)', '선택 설정', '— 음성·풋티지·팩트체크 등 추가 기능')
  );
  const optToggle = el('div', {
    id: 'optionalKeysToggle',
    style:
      'cursor:pointer;padding:14px 20px;background:var(--white);border:1px solid var(--bdr);border-radius:var(--r2);display:flex;align-items:center;justify-content:space-between;transition:all .2s',
  });
  ...
  optInfo.appendChild(
    el('div', {
      style: 'font-size:14px;font-weight:600;color:var(--t1)',
      textContent: '고급 설정 열기',
    })
  );
  optInfo.appendChild(
    el('div', {
      style: 'font-size:11px;color:var(--t4);margin-top:2px',
      textContent: 'TTS 음성 · ElevenLabs · Pexels 풋티지 · Perplexity 팩트체크 · 모델 선택',
    })
  );
  optLeft.appendChild(optInfo);
  optToggle.appendChild(optLeft);
  optToggle.appendChild(
    el('span', {
      id: 'optionalKeysArrow',
      style: 'font-size:14px;color:var(--t3);transition:transform .3s',
      textContent: '\u25BC',
    })
  );
  container.appendChild(optToggle);

  const optBody = el('div', {
    id: 'optionalKeysBody',
    style: 'display:' + (hasOptionalKeys ? 'block' : 'none'),
  });
  const optCard = el('div', {
    className: 'cd',
    style:
      'padding:24px;margin-top:2px;border-top:none;border-top-left-radius:0;border-top-right-radius:0',
  });
  optCard.appendChild(
    _mkKeyField(
      'Google TTS API Key',
      ...
      !!keys.tts,
      'AIza...',
      'Google Cloud Console \u2192 Cloud Text-to-Speech API 활성화'
    )
  );
  optCard.appendChild(
    _mkKeyField(
  ...
  const modelSection = el('div', {
    style: 'border-top:1px solid var(--bdr);padding-top:16px;margin-top:8px',
  });
  const geminiModelField = el('div', { className: 'field' });
  geminiModelField.appendChild(
    el('label', { style: 'font-weight:600', textContent: '\uD83C\uDFAC 영상 분석 AI 모델' })
  );
  geminiModelField.appendChild(
    _mkSelect(
      'geminiVideoModel',
      ...
      keys.geminiVideoModel === 'gemini-2.0-flash' || keys.geminiVideoModel === 'gemini-2.5-flash'
        ? 'gemini-2.5-pro'
        : keys.geminiVideoModel || 'gemini-2.5-pro'
    )
  );
  geminiModelField.appendChild(
    el('p', {
      style: 'font-size:11px;color:var(--t4);margin-top:4px;line-height:1.5',
      textContent: '영상 분석(Step 4)에서 사용할 AI 모델입니다.',
    })
  );
  modelSection.appendChild(geminiModelField);

  const claudeModelField = el('div', {
    className: 'field',
    id: 'claudeModelField',
    style: isClaude ? '' : 'display:none',
  });
  claudeModelField.appendChild(
    el('label', { style: 'font-weight:600', textContent: '\uD83E\uDDE0 Claude 대본 생성 모델' })
  );
  claudeModelField.appendChild(
    _mkSelect(
      'claudeModel',
      [
        { value: 'claude-sonnet-4-20250514', label: 'Claude Sonnet 4 (기본 — 빠르고 효율적)' },
        {
          ...
          label: 'Claude Opus 4 (프리미엄 — 더 깊은 분석, 속도 느림, 비용 높음)',
        },
      ],
  ...
  claudeModelField.appendChild(
    el('p', {
      style: 'font-size:11px;color:var(--t4);margin-top:4px;line-height:1.5',
      textContent: '대본 생성·팩트체크·풋티지 브리프에서 사용할 Claude 모델입니다.',
    })
  );
  ...
  const costCard = el('div', {
    style:
      'padding:14px 18px;background:rgba(184,138,0,.04);border:1px solid rgba(184,138,0,.15);border-radius:var(--r2);margin-top:16px',
  });
  const costHeader = el('div', {
    style: 'display:flex;align-items:center;gap:8px;margin-bottom:10px',
  });
  costHeader.appendChild(el('span', { style: 'font-size:16px', textContent: '\uD83D\uDCB0' }));
  costHeader.appendChild(
    el('span', {
      style: 'font-size:13px;font-weight:600;color:var(--t1)',
      textContent: 'API 비용 안내',
    })
  );
  ...
  [
    ...
    costBody.appendChild(el('div', { textContent: t }));
  });
  costBody.appendChild(
    el('div', {
      style:
        'margin-top:8px;padding-top:8px;border-top:1px solid rgba(184,138,0,.12);font-size:11px;color:var(--t3)',
      textContent:
        '\uD83D\uDCA1 필수 키(YouTube + AI 모델 + Google AI)만 사용하면 대부분 무료 범위에서 이용 가능합니다. 비용은 각자의 API 계정에서 직접 발생합니다.',
    })
  );
  ...
  // ═══ 저장 정책 안내 카드 ═══
  const policyCard = el('div', {
    style:
      'padding:14px 18px;background:var(--bg);border:1px solid var(--bdr);border-radius:var(--r2);margin-top:16px',
  });
  const policyHdr = el('div', {
    style: 'display:flex;align-items:center;gap:8px;margin-bottom:10px',
  });
  policyHdr.appendChild(el('span', { style: 'font-size:14px', textContent: '\uD83D\uDCCB' }));
  policyHdr.appendChild(
    el('span', {
      style: 'font-size:13px;font-weight:700;color:var(--t1)',
      textContent: '데이터 저장 안내',
    })
  );
  ...
  const policyItems = [
    ...
    { icon: '\uD83D\uDCC2', label: '히스토리', desc: 'ZIP 다운로드 완료 시 저장' },
    {
      icon: '\u26A0\uFE0F',
      label: '공용 PC 이용 시',
      desc: '공용 PC 모드 사용 또는 종료 전 로그아웃 + 키 삭제 권장',
    },
  ];
  policyItems.forEach((item) => {
    const row = el('div', {
      style: 'display:flex;align-items:center;gap:8px;padding:5px 0;font-size:12px',
    });
    row.appendChild(
      el('span', {
        style: 'font-size:12px;width:20px;text-align:center;flex-shrink:0',
        textContent: item.icon,
      })
    );
    row.appendChild(
      el('span', {
        style: 'font-weight:600;color:var(--t1);min-width:90px;flex-shrink:0',
  ...
  const secBox = el('div', {
    style:
      'padding:14px 18px;background:rgba(6,182,212,.06);border:1px solid rgba(6,182,212,.15);border-radius:var(--r2);margin-top:16px',
  });
  const secRow = el('div', {
    style: 'display:flex;align-items:flex-start;gap:10px;margin-bottom:10px',
  });
  secRow.appendChild(
    el('span', { style: 'font-size:16px;flex-shrink:0', textContent: '\uD83D\uDD12' })
  );
  ...
  secText.appendChild(
    el('div', {
      textContent: '회사 서버로 전송되지 않으며, API 비용은 각자의 계정에서 직접 발생합니다.',
    })
  );
  secText.appendChild(
    el('div', {
      textContent:
        '공용 PC에서는 사용을 권장하지 않습니다. 사용 후 반드시 아래 버튼으로 키를 삭제해주세요.',
    })
  );
  secText.appendChild(
    el('div', {
      textContent:
        '공용 PC 모드는 정상 종료·로그아웃 기준으로 흔적 삭제를 돕습니다. 강제 종료·OS 크래시 상황에서는 일부 흔적이 남을 수 있습니다.',
    })
  );
  ...
  const sessionRow = el('div', {
    style:
      'display:flex;align-items:center;gap:8px;margin-bottom:10px;padding:8px 12px;background:rgba(184,138,0,.06);border:1px solid rgba(184,138,0,.12);border-radius:var(--r)',
    id: 'sessionOnlyRow',
  });
  const sessionCheck = el('input', { id: 'sessionOnlyCheck' });
  sessionCheck.type = 'checkbox';
  sessionCheck.checked = _sessionOnlyMode;
  sessionCheck.style.cssText =
    'accent-color:var(--yel);cursor:pointer;width:16px;height:16px;flex-shrink:0';
  sessionRow.appendChild(sessionCheck);
  const sessionLabel = el('div', {
    style: 'font-size:12px;color:var(--t2);line-height:1.4;cursor:pointer',
  });
  sessionLabel.appendChild(
    el('strong', { style: 'color:var(--yel)', textContent: '공용 PC 모드' })
  );
  sessionLabel.appendChild(
    document.createTextNode(' — 정상 종료/로그아웃 시 API 키, 로그인, 작업 기록을 자동 삭제합니다')
  );
  sessionLabel.addEventListener('click', () => {
    sessionCheck.checked = !sessionCheck.checked;
    sessionCheck.dispatchEvent(new Event('change'));
  });
  sessionRow.appendChild(sessionLabel);
  secBox.appendChild(sessionRow);

  const secActions = el('div', {
    style:
      'display:flex;align-items:center;gap:8px;padding-top:10px;border-top:1px solid rgba(6,182,212,.12);flex-wrap:wrap',
  });
  secActions.appendChild(
    el('button', {
      className: 'btn bs',
      id: 'clearAllKeysBtn',
      style: 'font-size:11px;padding:5px 12px;color:var(--red);border-color:rgba(201,42,42,.25)',
      textContent: '\uD83D\uDDD1 전체 삭제',
    })
  );
  ...
  secActions.appendChild(
    el('span', { id: 'lastSavedInfo', style: 'font-size:11px;color:var(--t4);margin-left:auto' })
  );
  ...
  btnRow.appendChild(
    el('button', {
      className: 'btn bs btn-lg',
      id: 'validateKeysBtn',
      style: 'flex:0 0 auto;gap:6px',
      textContent: '\uD83D\uDD0D 연결 테스트',
    })
  );
  btnRow.appendChild(
...
export function bindFormEvents(validateAllKeys) {
  ...
  if (storageBadge) {
    ...
    if (isElectron && window.electronAPI.getStorageStatus) {
      storageBadge.style.cssText =
        'font-size:10px;font-weight:600;padding:2px 8px;border-radius:4px;background:var(--bg2);color:var(--t3)';
      ...
      window.electronAPI
        ...
        .then((status) => {
          if (status && status.encrypted) {
            storageBadge.style.cssText =
              'font-size:10px;font-weight:600;padding:2px 8px;border-radius:4px;background:rgba(13,146,84,.1);color:var(--grn)';
            storageBadge.textContent = '\uD83D\uDD12 OS 보안 저장소 (키체인/DPAPI)';
            const sr = $('sessionOnlyRow');
            if (sr) sr.style.display = 'none';
          } else {
            storageBadge.style.cssText =
              'font-size:10px;font-weight:600;padding:2px 8px;border-radius:4px;background:rgba(201,42,42,.1);color:var(--red)';
  ...
  if (sessionCheck) {
    sessionCheck.addEventListener('change', () => {
      ...
      if (_sessionOnlyMode)
        toast(
          '공용 PC 모드: 정상 종료/로그아웃 시 사용 흔적이 삭제됩니다. 강제 종료 시 일부 흔적이 남을 수 있습니다.'
        );
    });
  }
  ...
  if (clearBtn) {
    clearBtn.addEventListener('click', async () => {
      ...
      if (window.electronAPI && window.electronAPI.clearApiKeys)
        await window.electronAPI.clearApiKeys();
      try {
  ...
  if (exportBtn && window.electronAPI && window.electronAPI.exportApiKeys) {
    exportBtn.addEventListener('click', async () => {
      const pw = await promptModal(
        '내보내기 비밀번호를 설정하세요\n가져올 때 동일한 비밀번호가 필요합니다.',
        { placeholder: '4자 이상 비밀번호', confirmText: '내보내기', inputType: 'password' }
      );
      if (!pw) return;
      if (pw.length < 4) {
        toast('비밀번호는 4자 이상이어야 합니다', 'err');
        return;
      }
  ...
  if (importBtn && window.electronAPI && window.electronAPI.importApiKeys) {
    importBtn.addEventListener('click', async () => {
      const pw = await promptModal('내보낼 때 설정한 비밀번호를 입력하세요.', {
        placeholder: '비밀번호 입력',
        confirmText: '가져오기',
        inputType: 'password',
      });
...
async function _showUnsafeStorageWarning() {
  ...
  try {
    if (sessionStorage.getItem('_unsafe_storage_warned')) return;
  } catch (e) {}
  _unsafeStorageWarningShown = true;
  try {
    sessionStorage.setItem('_unsafe_storage_warned', '1');
  } catch (e) {}
