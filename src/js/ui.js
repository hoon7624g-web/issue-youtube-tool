...
export function renderVidList(vs) {
  ...
  vs.forEach((v, idx) => {
    ...
    h3.style.cssText =
      'font-size:15px;font-weight:600;margin-bottom:5px;line-height:1.4;letter-spacing:-.2px';
    ...
    if (v.planned) {
      const b = document.createElement('span');
      b.className = 'bdg';
      b.style.cssText = 'background:rgba(139,92,246,.1);color:#8B5CF6';
      b.textContent = '기획형';
      metaRow.appendChild(b);
    }
    if (parseFloat(r) > 10) {
    ...
    if (v.durText) {
      const b = document.createElement('span');
      b.className = 'bdg';
      b.style.cssText = 'background:rgba(107,114,128,.1);color:#6B7280;font-family:var(--mono)';
      b.textContent = v.durText;
      metaRow.appendChild(b);
    }
    if (v.durSec >= 1200) {
      const b = document.createElement('span');
      b.className = 'bdg';
      b.style.cssText = 'background:rgba(220,38,38,.1);color:#DC2626;font-size:10px';
      b.textContent = '20분+ 분석 시간 김';
      metaRow.appendChild(b);
    }
    ...
    statRow.appendChild(
      Object.assign(document.createElement('span'), { textContent: '\u25B6 ' + fmt(v.views) })
    );
    statRow.appendChild(
      Object.assign(document.createElement('span'), { textContent: '구독 ' + fmt(v.subs) })
    );
    statRow.appendChild(
      Object.assign(document.createElement('span'), { textContent: '구독 대비 ' + r + '배' })
    );
  ...
  vl.onclick = (e) => {
    const c2 = e.target.closest('.vc');
    if (!c2) return;
    const v = S.search.vids.find((x) => {
      return x.id === c2.dataset.id;
    });
    sSet({ [K.VIDEO_SV]: v });
    sNext();
  };
...
export function filterDuration(dur) {
  document.querySelectorAll('[data-dur]').forEach((x) => {
  ...
  ldDiv.appendChild(
    document.createTextNode(
      (dur === 'long' ? '롱폼' : dur === 'short' ? '숏폼' : '전체') + ' 영상을 검색하고 있습니다...'
    )
  );
  ...
  Api.getVids(
    ...
    dur,
    S.search.filterPeriod || '7d'
  )
    .then((vs) => {
      if (searchId !== _currentSearchId) return;
      sSet({ [K.SEARCH_VIDS]: vs });
      renderVidList(vs);
    })
    .catch((e) => {
      if (searchId !== _currentSearchId) return;
...
export function buildSb() {
  ...
  STEP_GROUPS.forEach((group) => {
    const groupHeader = el('div', {
      style: 'display:flex;align-items:center;gap:6px;padding:10px 12px 4px;margin-top:4px',
    });
    groupHeader.appendChild(
      el('span', {
        style:
          'font-size:10px;font-weight:700;text-transform:uppercase;letter-spacing:1px;color:var(--t4)',
    ...
    if (group.auto) {
      groupHeader.appendChild(
        el('span', {
          style:
            'font-size:8px;padding:1px 6px;border-radius:4px;background:rgba(37,99,235,.08);color:var(--blu);font-weight:600;letter-spacing:.5px',
          textContent: '자동',
        })
      );
    }
  ...
  const extras = el('div', {
    style: 'border-top:1px solid var(--bdr);margin-top:12px;padding-top:12px',
  });
  const menuItems = [
  ...
  menuItems.forEach((m) => {
    const nv = el('div', {
      className: 'nv',
      id: m.id,
      style: 'cursor:pointer;font-size:13px;color:var(--t3)',
    });
    nv.appendChild(
      el('span', {
        className: 'sn',
    ...
    if (m.hint) {
      labelWrap.appendChild(
        el('span', { style: 'font-size:9px;color:var(--t4);line-height:1', textContent: m.hint })
      );
    }
  ...
  const shortcutHint = el('div', {
    style:
      'margin-top:12px;padding:8px 12px;font-size:10px;color:var(--t4);line-height:1.6;border-top:1px solid var(--bdr)',
  });
  shortcutHint.appendChild(
    el('div', { style: 'font-weight:600;margin-bottom:2px', textContent: '\u2328\uFE0F 단축키' })
  );
  ...
  nav.onclick = (e) => {
    const target = e.target.closest('.nv');
    if (!target) return;
    ...
    if (target.id === 'btnHistory') {
      document.querySelectorAll('.nv').forEach((n) => n.classList.remove('ac'));
      target.classList.add('ac');
      runAction('openHistory');
      return;
    }
    if (target.id === 'btnApiKeySetting') {
      document.querySelectorAll('.nv').forEach((n) => n.classList.remove('ac'));
      target.classList.add('ac');
      runAction('openApiKeySettings');
      return;
    }
    if (target.id === 'btnMySettings') {
      document.querySelectorAll('.nv').forEach((n) => n.classList.remove('ac'));
      target.classList.add('ac');
      runAction('openMySettings');
      return;
    }
    if (!target.classList.contains('lk') && target.dataset.s) {
      ...
      const isSpecialPanelOpen = [
        ...
        const btn = $(id);
        return btn && btn.classList.contains('ac');
      });
...
export function syncSb() {
  ...
  if (S.nav.step === 1) {
    $('app').classList.add('no-sb');
    $('sidebar').style.display = 'none';
  } else {
    $('app').classList.remove('no-sb');
  ...
  if (ab) {
    ...
    const reqItems = [
      ...
      {
        label:
          _resolvedLlm === 'gemini' ? 'Gemini' : _resolvedLlm === 'chatgpt' ? 'ChatGPT' : 'Claude',
        ok: !!(keys.claude || keys.gemini || keys.openai),
      },
    ...
    const reqBadge = el('span', {
      className: 'bdg',
      style:
        'font-size:9px;padding:2px 8px;cursor:default;background:' +
        (reqOk === reqTotal ? 'rgba(13,146,84,.1)' : 'rgba(201,42,42,.1)') +
        ';color:' +
        (reqOk === reqTotal ? 'var(--grn)' : 'var(--red)'),
      textContent:
        reqOk === reqTotal ? '✓ 필수 준비 완료' : '필수 ' + reqOk + '/' + reqTotal + ' 미완료',
    });
  ...
  const pt = $('progText');
  if (pt) pt.textContent = Math.max(0, step - 1) + '/9';
  const ps = $('progStatus');
  if (ps) ps.textContent = PROG_MSG[Math.max(0, step - 2)] || '';
}
...
export function buildPanels() {
  ...
  main.textContent = '';
  STEPS.forEach((s) => {
    main.appendChild(el('div', { className: 'pnl', id: 'p' + s.n }));
  });
...
export function renderLogin() {
  ...
  const LOGO_SRC = document.querySelector('.logo-icon img')
  ...
  function _linkBtn(id, text) {
    return el('button', {
      id: id,
      style:
        'background:none;border:none;color:var(--acc);font-size:13px;cursor:pointer;font-family:var(--f);font-weight:500',
      textContent: text,
    });
  }
  ...
  function render() {
    ...
    if (mode === 'login') {
      ...
      box.appendChild(
        el('h2', { className: 'login-title', textContent: '유튜브도사 영상 제작 솔루션' })
      );
      ...
      lbtn.addEventListener('click', () => {
        lbtn.disabled = true;
        lbtn.textContent = '로그인 중...';
        $('lerr').style.display = 'none';
        Api.login($('lEmail').value, $('lPw').value)
          .then((u) => {
            sSet({ [K.AUTH_USER]: u });
            ...
            if (result && result.restored) {
              ...
              showP();
              const stepName =
                (STEPS.find((s) => s.n === S.nav.step) || {}).l || S.nav.step + '단계';
      ...
      p.appendChild(wrap);
      $('lPw').addEventListener('keydown', (e) => {
        if (e.key === 'Enter') lbtn.click();
      });
      setTimeout(() => {
        const f = $('lEmail');
        if (f) f.focus();
      ...
      box.appendChild(
        el('p', {
          className: 'login-sub',
          textContent: '가입 후 관리자 승인이 완료되면 이용할 수 있습니다',
        })
      );
      ...
      const sokBox = el('div', {
        id: 'sok',
        style:
          'color:var(--grn);font-size:12px;margin-bottom:12px;display:none;padding:8px 12px;background:var(--grn-bg);border-radius:var(--r)',
      });
      ...
      sbtn.addEventListener('click', () => {
        ...
        if (pw.length < CONFIG.PW_MIN_LENGTH) {
          ...
          sbtn.disabled = false;
          sbtn.textContent = '가입 신청';
          return;
        }
        if (
          (CONFIG.PW_REQUIRE_ALPHA && !/[a-zA-Z]/.test(pw)) ||
          (CONFIG.PW_REQUIRE_DIGIT && !/[0-9]/.test(pw))
        ) {
        ...
        authSignup($('sEmail').value, pw, $('sName').value, $('sPhone').value, $('sCohort').value)
          .then((r) => {
            const sokEl = $('sok');
            sokEl.textContent = '';
            ...
            sokEl.appendChild(
              el('div', {
                style: 'font-size:11px;color:var(--t2);line-height:1.5',
                textContent:
                  '관리자 승인 완료 시 카카오톡 또는 이메일로 안내드립니다. 운영 시간 내 순차 승인되며, 승인 전에는 로그인이 되지 않으니 잠시만 기다려주세요.',
              })
            );
            const goLoginBtn = el('button', {
              className: 'btn bp',
              style: 'margin-top:10px;font-size:13px',
              textContent: '로그인 화면으로 이동',
            });
          ...
          .catch((e) => {
            $('serr').textContent = friendlyError(e);
            $('serr').style.display = 'block';
            sbtn.disabled = false;
            sbtn.textContent = '가입 신청';
          });
...
export function toggleTheme() {
  ...
  if (appEl)
    setTimeout(() => {
      appEl.classList.remove('theme-transitioning');
    }, 400);
}
