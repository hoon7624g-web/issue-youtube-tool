...
export function getMaxPromptChars() {
  return CONFIG.MAX_PROMPT_CHARS;
}
export function getMaxPromptCharsPro() {
  return CONFIG.MAX_PROMPT_CHARS_PRO;
}
export function getMaxOutputTokens() {
  return CONFIG.MAX_OUTPUT_TOKENS;
}
export function getMaxOutputTokensShort() {
  return CONFIG.MAX_OUTPUT_TOKENS_SHORT;
}
...
export async function fetchWithTimeout(
  ...
  const onAbort = () => {
    try {
      controller.abort(signal && signal.reason ? signal.reason : new Error('aborted'));
    } catch (_) {}
  };
  ...
  try {
    ...
    if (_isAbortError(e) && signal && signal.aborted)
...
function _saveThrottleCalls(calls) {
  try {
    sessionStorage.setItem(_THROTTLE_STORAGE_KEY, JSON.stringify(calls));
  } catch (_) {}
}
let _apiCalls = _loadThrottleCalls();
export function checkThrottle() {
  const now = Date.now();
  _apiCalls = _apiCalls.filter((t) => {
    return now - t < CONFIG.API_RATE_WINDOW;
  });
...
export function initSession() {
  ...
  // 웹 환경
  try {
    const s = JSON.parse(localStorage.getItem('yt_session'));
    if (s && s.access_token) {
      _sessionCache = s;
      _startRefreshInterval();
...
export function onSessionStorageFail(cb) {
  _onSessionStorageFail = cb;
}
  ...
  if (window.electronAPI && window.electronAPI.setSession) {
    ...
    window.electronAPI
      ...
      .then((ok) => {
        if (!ok) {
          console.warn(
            '[Session] safeStorage 저장 실패 — 메모리 전용 모드 (앱 종료 시 재로그인 필요)'
          );
    ...
    try {
      localStorage.setItem('yt_session', JSON.stringify(s));
    } catch (e) {}
  }
}
...
export function initApiKeys() {
  ...
  try {
    const k = JSON.parse(localStorage.getItem('yt_api_keys'));
    if (k) {
      _keyCache = k;
    }
  } catch (e) {}
    ...
    return window.electronAPI
      ...
      .then((ok) => {
        if (!ok) {
          ...
          return {
            ok: false,
            method: 'safeStorage_unavailable',
            error: 'OS 보안 저장소를 사용할 수 없어 API 키를 저장할 수 없습니다.',
          };
        }
      ...
      .catch((e) => {
        ...
        return {
          ok: false,
          method: 'ipc_error',
          error: 'API 키 저장 중 오류가 발생했습니다. 앱을 다시 실행해주세요.',
        };
      });
  }
  _keyCache = keys;
...
export function hasYtKey() {
  return !!getApiKeys().youtube;
}
...
export async function proxyFetch(endpoint, options) {
  ...
  options.headers['X-App-Client'] =
    window.electronAPI && window.electronAPI.isElectron ? 'electron' : 'dev';
...
function _authHeaders() {
  return {
    ...
    'X-App-Client': window.electronAPI && window.electronAPI.isElectron ? 'electron' : 'dev',
  };
}
...
export async function authLogin(email, password) {
  ...
  if (d.user && d.user.approval_status !== '승인완료')
