...
import {
  checkThrottle, getApiKeys, hasYtKey, hasKey, proxyFetch,
  TTS_CHUNK_SIZE, TTS_CHUNK_MIN_BREAK, fetchWithTimeout
...
export async function ytFetch(endpoint, params) {
  ...
  if (r.status === 403) {
    try {
      ...
      if (reason === 'quotaExceeded') throw new Error('YouTube API 일일 할당량을 초과했습니다. 한국 시간 오후 4시경(태평양 자정)에 초기화됩니다.');
    } catch (e) { if (e.message.includes('할당량')) throw e; }
    throw new Error('YouTube API 키가 유효하지 않습니다. 설정을 확인해주세요.');
  }
...
export async function genElevenLabs(text, voiceId, { signal } = {}) {
  ...
  if (window.electronAPI && window.electronAPI.callElevenLabsTTS) {
    ...
    const r = await _invokeAbortableMedia(
      (requestId) => window.electronAPI.callElevenLabsTTS(text.substring(0, 5000), voiceId, elSpeed, requestId),
      (requestId) => window.electronAPI.cancelElevenLabsRequest ? window.electronAPI.cancelElevenLabsRequest(requestId) : Promise.resolve(),
      signal
    );
    ...
    return { url: URL.createObjectURL(blob), blob, dur: Math.round(text.length / 6), provider: 'ElevenLabs', voiceName: '프리미엄 음성' };
  }
  _warnWebFallback('ElevenLabs');
  const r = await fetchWithTimeout('https://api.elevenlabs.io/v1/text-to-speech/' + voiceId, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json', 'xi-api-key': keys.elevenlabs },
    body: JSON.stringify({ text: text.substring(0, 5000), model_id: 'eleven_multilingual_v2', voice_settings: { stability: 0.65, similarity_boost: 0.6, style: 0.25, use_speaker_boost: true }, speed: S.voice.voiceSpeed || 1.0 }),
    signal,
  }, 120000, signal);
  if (r.status === 401) throw new Error('ElevenLabs API 키가 유효하지 않습니다.');
  if (r.status === 402) throw new Error('ElevenLabs 크레딧이 부족합니다. elevenlabs.io에서 무료 한도를 확인하거나 크레딧을 충전해주세요.');
  if (r.status === 429) throw new Error('ElevenLabs 요청 한도를 초과했습니다. 잠시 후 다시 시도해주세요.');
...
export async function uploadToElevenLabs(file) {
  ...
  if (window.electronAPI && window.electronAPI.uploadElevenLabsVoice) {
    ...
    if (!result || !result.ok) throw new Error((result && result.error) || 'ElevenLabs 업로드 실패');
  ...
  const r = await fetch('https://api.elevenlabs.io/v1/voices/add', {
    method: 'POST', headers: { 'xi-api-key': keys.elevenlabs }, body: formData
  });
...
export function patchApi(ApiObj) {
  ...
  ApiObj.getTrends = async () => {
    ...
    try {
      ...
      const d = await r.json();
      const result = { zum: d.zum || [], nate: d.nate || [], google: d.google || [], source: d.source || 'adsensefarm' };
      cacheSet('realtimeKw', result);
      return result;
    } catch (e) { console.error('[RealtimeKW]', e.message); return { zum: [], nate: [], google: [], source: 'error' }; }
  };
  ...
  ApiObj.getVids = async (kwLabels, duration, period) => {
    ...
    try {
      ...
      const searchParams = { part: 'snippet', q: mergedQ, type: 'video', order: 'viewCount', publishedAfter: since, maxResults: 50, regionCode: 'KR', relevanceLanguage: 'ko' };
      ...
      // 중복 제거
      const seen = {};
      allItems = allItems.filter(i => { const vid = i.id.videoId; if (vid && !seen[vid]) { seen[vid] = true; return true; } return false; });
      ...
      return scoreVids(vd.items.map(it => {
        ...
        const hm = durStr.match(/(\d+)H/); if (hm) durSec += parseInt(hm[1]) * 3600;
        const mm = durStr.match(/(\d+)M/); if (mm) durSec += parseInt(mm[1]) * 60;
        const sm = durStr.match(/(\d+)S/); if (sm) durSec += parseInt(sm[1]);
        let durText = '';
        if (durSec >= 3600) durText = Math.floor(durSec / 3600) + ':' + String(Math.floor((durSec % 3600) / 60)).padStart(2, '0') + ':' + String(durSec % 60).padStart(2, '0');
        else durText = Math.floor(durSec / 60) + ':' + String(durSec % 60).padStart(2, '0');
        return { id: it.id, title: it.snippet.title, ch: it.snippet.channelTitle, thumb: (it.snippet.thumbnails.high || it.snippet.thumbnails.default).url, date: it.snippet.publishedAt.substring(0, 10), views: parseInt(it.statistics.viewCount || 0), likes: parseInt(it.statistics.likeCount || 0), subs: cm[it.snippet.channelId] || 0, desc: it.snippet.description || '', score: 0, news: false, durSec, durText };
      }).filter(v => {
  ...
  ApiObj.genVoice = async (text, voiceId, { signal } = {}) => {
    ...
    if (voiceId === 'custom' && S.voice.elVoiceId) return genElevenLabs(script, S.voice.elVoiceId, { signal });
    if (voiceId === 'el-custom' && S.voice.customElVoiceId) return genElevenLabs(script, S.voice.customElVoiceId, { signal });
    ...
    if (keys.tts) {
      ...
      while (remaining.length > 0) {
        if (remaining.length <= TTS_CHUNK_SIZE) { chunks.push(remaining); break; }
      ...
      async function callTTS(chunk) {
        ...
        if (window.electronAPI && window.electronAPI.callTTS) {
          const r = await _invokeAbortableMedia(
            (requestId) => window.electronAPI.callTTS(chunk, voice.name, voice.gender, speed, requestId),
            (requestId) => window.electronAPI.cancelTTSRequest ? window.electronAPI.cancelTTSRequest(requestId) : Promise.resolve(),
            signal
          );
          if (r && r.cancelled) throw _abortError();
          if (!r) throw new Error('TTS IPC 응답 없음');
          if (r.status === 403) throw new Error('Google TTS API 키가 유효하지 않거나 API가 활성화되지 않았습니다');
          if (r.error) throw new Error(r.error);
          if (!r.audioContent) throw new Error('TTS 응답에 오디오 데이터가 없습니다 (status: ' + r.status + ')');
          return b64toBlob(r.audioContent, 'audio/mp3');
        }
        _warnWebFallback('Google TTS');
        const r = await fetchWithTimeout('https://texttospeech.googleapis.com/v1/text:synthesize?key=' + keys.tts, {
          method: 'POST', headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ input: { text: chunk }, voice: { languageCode: 'ko-KR', name: voice.name, ssmlGender: voice.gender }, audioConfig: { audioEncoding: 'MP3', speakingRate: speed, pitch: 0 } }),
          signal,
        }, 120000, signal);
        if (r.status === 403) throw new Error('Google TTS API 키가 유효하지 않거나 API가 활성화되지 않았습니다');
        if (!r.ok) { const d = await r.json(); throw new Error(d.error && d.error.message || 'TTS 오류: ' + r.status); }
        const d = await r.json();
  ...
  ApiObj.genThumb = async (title, script) => {
    if (!hasKey('llm')) return ['충격! ' + title, '이것만 알면 인생이 바뀝니다', '아무도 몰랐던 진실'];
    const prompt = '당신은 유튜브 썸네일 카피라이터입니다.\n\n[영상 제목]\n' + title + '\n\n[대본 요약]\n' + (script || '').substring(0, 500) + '\n\n[규칙]\n- 각 문구는 15자 이내\n- 호기심, 충격, 반전\n\nJSON 배열로만 응답: ["문구1","문구2","문구3"]';
    const t = await callLLM(prompt);
    try { return JSON.parse(t.replace(/```json|```/g, '').trim()); } catch (e) { return [t.substring(0, 30)]; }
  };
...
export function setupVoiceHandlers() {
  registerAction('playVoicePreview', async (btn) => {
    if (shared.previewAudio && !shared.previewAudio.paused) {
      ...
      if (shared.previewAudio.src && shared.previewAudio.src.startsWith('blob:')) try { URL.revokeObjectURL(shared.previewAudio.src); } catch (e) {}
      ...
      if (bars) for (let i = 0; i < bars.children.length; i++) bars.children[i].style.height = '6px';
      return;
    }
    ...
    const sampleText = '여러분, 오늘 정말 놀라운 이야기를 들려드리려고 합니다. 최근 화제가 된 이 사건, 한번 자세히 살펴볼까요?';
    try {
      let audio;
      if (provider === 'elevenlabs') {
        ...
        const keys = getApiKeys();
        if (!keys.tts) { toast('Google TTS API 키를 설정해주세요', 'err'); btn.querySelector('span').textContent = '▶'; document.getElementById('vpTime').textContent = '미리듣기'; btn.disabled = false; return; }
        // Electron: Main IPC로 키 노출 방지
        if (window.electronAPI && window.electronAPI.callTTS) {
          const r = await window.electronAPI.callTTS(sampleText, voiceRef, 'FEMALE', S.voice.voiceSpeed || 1.0);
          ...
          const r = await fetchWithTimeout('https://texttospeech.googleapis.com/v1/text:synthesize?key=' + keys.tts, {
            method: 'POST', headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ input: { text: sampleText }, voice: { languageCode: 'ko-KR', name: voiceRef }, audioConfig: { audioEncoding: 'MP3', speakingRate: S.voice.voiceSpeed || 1.0, pitch: 0 } })
          }, 120000);
          const d = await r.json();
      ...
      if (shared.previewAudio && shared.previewAudio.src && shared.previewAudio.src.startsWith('blob:')) try { URL.revokeObjectURL(shared.previewAudio.src); } catch (e) {}
      shared.previewAudio = audio; audio.play();
      btn.disabled = false;
      ...
      shared.previewAnimId = setInterval(() => {
        if (!shared.previewAudio) { clearInterval(shared.previewAnimId); shared.previewAnimId = null; return; }
        for (let i = 0; i < bars.length; i++) { bars[i].style.height = (6 + Math.random() * 14) + 'px'; bars[i].style.background = shared.previewAudio.paused ? 'var(--bg3)' : 'var(--acc)'; }
      }, 150);
      audio.onended = () => { clearInterval(shared.previewAnimId); shared.previewAnimId = null; btn.querySelector('span').textContent = '▶'; document.getElementById('vpTime').textContent = '미리듣기'; for (let i = 0; i < bars.length; i++) bars[i].style.height = '6px'; };
    } catch (e) { toast(friendlyError(e), 'err'); btn.querySelector('span').textContent = '▶'; document.getElementById('vpTime').textContent = '미리듣기'; btn.disabled = false; }
  });

  registerAction('handleVoiceUpload', async (input) => {
    const file = input.files[0]; if (!file) return;
    const keys = getApiKeys();
    if (!keys.elevenlabs) { document.getElementById('uploadStatus').textContent = 'ElevenLabs 키를 먼저 설정해주세요'; document.getElementById('uploadStatus').style.color = 'var(--red)'; return; }
    ...
    } catch (e) { document.getElementById('uploadStatus').textContent = friendlyError(e); document.getElementById('uploadStatus').style.color = 'var(--red)'; }
  });
}
