// ═══════════════════════════════════════════════
// main.js — Electron Main Process (v3.6.2)
...
try {
  ...
  log.warn('[Update] electron-updater not installed');
  autoUpdater = null;
}
  ...
  if (app.isPackaged && process.platform === 'darwin') {
    const checkMacUpdate = async () => {
      ...
      const res = await httpsGet(
        'https://api.github.com/repos/hoon7624g-web/issue-youtube-tool/releases/latest',
        {
      ...
      try {
        ...
        if (latestVer && latestVer !== currentVer && _isNewerVersion(latestVer, currentVer)) {
          ...
          updateStatus = {
            ...
            version: latestVer,
            error: null,
            macDownloadUrl: data.html_url || '',
          };
          sendUpdateStatus();
        }
      } catch (e) {
        log.warn('[Update:macOS] 파싱 실패:', e.message);
      }
    };
...
app.on('before-quit', () => {
  ...
  try {
    ...
    log.warn('[SessionOnly] KEY_FILE 삭제 실패:', e.message);
  }
  try {
    if (fs.existsSync(SESSION_FILE)) fs.unlinkSync(SESSION_FILE);
  } catch (e) {
    log.warn('[SessionOnly] SESSION_FILE 삭제 실패:', e.message);
  }
});
...
ipcMain.handle('open-update-page', (event) => {
  assertTrustedSender(event);
  const url =
    updateStatus.macDownloadUrl ||
    'https://github.com/hoon7624g-web/issue-youtube-tool/releases/latest';
  shell.openExternal(url);
  return { ok: true };
});
...
ipcMain.handle('open-perplexity', (event, query) => {
  ...
  if (perplexityWin && !perplexityWin.isDestroyed()) {
...
ipcMain.handle('open-api-guide', (event) => {
  ...
  if (apiGuideWin && !apiGuideWin.isDestroyed()) {
    apiGuideWin.focus();
    return;
  }
          ...
          if (
            ...
            try {
              ...
              if (
                ...
                !(
                  redirectUrl.hostname === 'issuelink.co.kr' ||
                  redirectUrl.hostname.endsWith('.issuelink.co.kr')
                )
              ) {
                _reject(new Error('Untrusted redirect: ' + redirectUrl.hostname));
                return;
              }
              ...
              fetchWithRedirects(redirectUrl.href, maxRedirects - 1)
                .then(_resolve)
                .catch(_reject);
            } catch (e) {
