...
export async function handleGAS(
  ...
  if (!['issuelink', 'subtitle'].includes(action))
    return json(cors, { error: '허용되지 않은 action입니다' }, 400);
  const allowedParams: Record<string, string[]> = {
    issuelink: ['action', 'cat'],
    subtitle: ['action', 'videoId'],
  };
  const safeParams = new URLSearchParams();
  (allowedParams[action] || []).forEach((k) => {
    const v = url.searchParams.get(k);
    if (v) safeParams.set(k, v);
  });
  ...
  if (action === 'issuelink') {
    ...
    try {
      const { data: cached } = await svc
        ...
        .single();
      if (cached) {
        await logUsage(svc, userId, 'gas_cache', 200, 0);
    ...
    if (!resp.ok)
      return json(cors, { error: 'GAS API 오류', code: 'UPSTREAM_ERROR', detail: data }, 502);
