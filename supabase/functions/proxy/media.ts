...
export async function handleTTS(
  ...
  const sanitizedBody = {
    ...
    voice: {
      languageCode: body.voice?.languageCode || 'ko-KR',
      name: body.voice?.name,
      ssmlGender: body.voice?.ssmlGender,
    },
    audioConfig: {
      audioEncoding: body.audioConfig?.audioEncoding || 'MP3',
      speakingRate: Math.min(Math.max(body.audioConfig?.speakingRate || 1.0, 0.5), 2.0),
      pitch: Math.min(Math.max(body.audioConfig?.pitch || 0, -10), 10),
    },
  };
  const resp = await fetch(
    `https://texttospeech.googleapis.com/v1/text:synthesize?key=${Deno.env.get('GOOGLE_TTS_KEY')}`,
    {
      method: 'POST',
  ...
  if (!resp.ok) {
    notifySlack('tts', resp.status, `TTS API ${resp.status}`, userId);
    return json(
      cors,
      { error: 'TTS API 오류', code: 'UPSTREAM_ERROR', upstream_status: resp.status, detail: data },
      502
    );
  }
...
export async function handleElevenLabs(
  cors: Record<string, string>,
  path: string,
  req: Request,
  svc: any,
  ...
  if (sub.startsWith('tts/')) {
    ...
    let elBody: any;
    try {
      elBody = await req.json();
    } catch (_) {
      return json(cors, { error: 'Invalid request body' }, 400);
    }
    if (!elBody.text || typeof elBody.text !== 'string')
      return json(cors, { error: '텍스트가 필요합니다' }, 400);
    if (elBody.text.length > 5000)
      return json(cors, { error: '텍스트가 너무 깁니다 (5000자 제한)' }, 400);
    const sanitizedEl = {
      text: elBody.text,
    ...
    if (!resp.ok) {
      notifySlack(
        'elevenlabs',
        resp.status,
        `ElevenLabs TTS ${resp.status} voice:${voiceId}`,
        userId
      );
      return json(
        cors,
        { error: '음성 생성 오류', code: 'UPSTREAM_ERROR', upstream_status: resp.status },
        502
      );
    }
    return rawResponse(
      cors,
      resp.body,
      { ...rh, 'Content-Type': resp.headers.get('Content-Type') || 'audio/mpeg' },
      200
    );
  }
  if (sub === 'voices/add') {
    ...
    if (contentLength > 10 * 1024 * 1024)
      return json(cors, { error: '파일이 너무 큽니다 (10MB 제한)' }, 400);
    ...
    if (!resp.ok)
      return json(
        cors,
        { error: 'ElevenLabs API 오류', code: 'UPSTREAM_ERROR', detail: data },
        502
      );
    return json(cors, data, 200, rh);
  }
