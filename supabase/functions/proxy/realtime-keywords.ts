...
async function fetchKeywords(endpoint: string): Promise<{ keywords: { keyword: string; rank: number }[]; time: string }> {
  try {
    const resp = await fetch(BASE + endpoint, { headers: HEADERS, signal: AbortSignal.timeout(10000) });
    if (!resp.ok) throw new Error(endpoint + " HTTP " + resp.status);
    const d = await resp.json();
    if (d.result !== "success" || !Array.isArray(d.data)) return { keywords: [], time: "" };
    const keywords = d.data.filter((kw: any) => kw && typeof kw === 'string').slice(0, 10).map((kw: string, i: number) => ({ keyword: kw.trim(), rank: i + 1 }));
...
export async function handleRealtimeKeywords(cors: Record<string, string>) {
  ...
  try {
    const { data: cached } = await svc.from("youtube_cache")
      .select("result").eq("cache_key", "realtime-kw:kr")
  ...
  if (result.count > 0) {
    try {
      await svc.from("youtube_cache").upsert({
        cache_key: "realtime-kw:kr", endpoint: "realtime-keywords", result,
