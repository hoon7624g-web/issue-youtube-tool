...
export async function handleAdmin(cors: Record<string, string>, path: string, req: Request, svc: any) {
  if (path === "/admin/users") {
  ...
  if (path === "/admin/approve" && req.method === "POST") {
    ...
    if (!body.user_id || !/^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(body.user_id)) return json(cors, { error: "유효하지 않은 사용자 ID" }, 400);
  ...
  if (path === "/admin/usage") {
    ...
    if (!logs) return json(cors, { endpoints: {}, topUsers: [], daily: [], totalCalls: 0, estimatedCost: 0 });
    ...
    for (const [ep, times] of Object.entries(epResponseTimes)) {
      if (times.length >= 2) {
        percentiles[ep] = { p50: Math.round(percentile(times, 50)), p95: Math.round(percentile(times, 95)), count: times.length };
      }
    }
    ...
    const costMap: Record<string, number> = {
      llm: 0.015, tts: 0.006, elevenlabs: 0.03, youtube: 0.0001, gas: 0,
    };
    let estimatedCost = 0;
    ...
    return json(cors, {
      totalCalls: logs.length, endpoints, topUsers, daily, percentiles,
  ...
  if (path === "/admin/rate-config") {
    if (req.method === "POST") {
      ...
      if (body.max_requests < 1 || body.max_requests > 10000) return json(cors, { error: "max_requests는 1~10000 범위" }, 400);
      if (body.window_seconds < 60 || body.window_seconds > 86400) return json(cors, { error: "window_seconds는 60~86400 범위" }, 400);
  ...
  if (path === "/admin/demo-bypass" && req.method === "POST") {
    ...
    const { data, error } = await svc.from("demo_bypass").upsert({
      user_id: body.user_id, active: true, expires_at: expiresAt,
      created_by: "admin", note: body.note || "웨비나 데모",
  ...
  if (path === "/admin/demo-bypass" && req.method === "GET") {
    const { data } = await svc.from("demo_bypass").select("*").eq("active", true).gt("expires_at", new Date().toISOString());
    return json(cors, data || []);
  }
  ...
  if (path === "/admin/cleanup" && req.method === "POST") {
    ...
    const { count: targetCount } = await svc.from("usage_logs")
      .select("*", { count: "exact", head: true }).lt("created_at", cutoff);

    if (!targetCount || targetCount === 0) {
      return json(cors, { message: "삭제할 로그가 없습니다", deleted: 0, cutoff_date: cutoff.substring(0, 10) });
    }

    let totalDeleted = 0;
    const BATCH = 1000;
    for (let i = 0; i < 100; i++) {
      const { data: batch, error } = await svc.from("usage_logs")
        .delete().lt("created_at", cutoff).limit(BATCH).select("id");
      if (error) { console.error("[Cleanup] batch error:", error.message); break; }
    ...
    return json(cors, {
      ...
      deleted: totalDeleted, cutoff_date: cutoff.substring(0, 10), target_count: targetCount,
    });
  }

  // ── 만료된 youtube_cache 정리 ──
  if (path === "/admin/cleanup-cache" && req.method === "POST") {
    const { data: expired, error } = await svc.from("youtube_cache")
      .delete().lt("expires_at", new Date().toISOString()).select("cache_key");
    if (error) return json(cors, { error: error.message }, 500);
    return json(cors, { message: `만료 캐시 ${expired?.length || 0}건 삭제`, deleted: expired?.length || 0 });
  }
