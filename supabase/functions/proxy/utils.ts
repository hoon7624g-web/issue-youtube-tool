...
export function getCorsHeaders(req: Request) {
  ...
  if (origin === 'null') {
    ...
    if (!(appClient === 'electron' && isElectronUA) && !isDevClient) {
      ...
      console.warn(
        '[CORS] Rejecting null origin — X-App-Client:',
        appClient,
        'UA:',
        ua.substring(0, 60)
      );
...
export async function validateUser(authHeader: string) {
  ...
  try {
    ...
    const result =
      profile.approval_status !== '승인완료' ? { ...profile, rejected: true } : profile;
...
const DEFAULT_LIMITS: Record<string, { limit: number; window: number }> = {
  youtube: { limit: 100, window: 3600 },
  llm: { limit: 60, window: 3600 },
  tts: { limit: 30, window: 3600 },
  elevenlabs: { limit: 20, window: 3600 },
  gas: { limit: 60, window: 3600 },
...
export async function checkRate(svc: any, userId: string, endpoint: string, userRole?: string) {
  const bypassed = {
    ...
    bypassed: true,
    nearLimit: false,
    rateHeaders: {} as Record<string, string>,
  };
  if (userRole === 'admin') return bypassed;
  try {
    const { data: bypass } = await svc
      .from('demo_bypass')
      .select('id')
      .eq('user_id', userId)
      .eq('active', true)
...
//
const _errorBuckets: Record<
  string,
  { count: number; firstAt: number; lastNotified: number; samples: string[] }
> = {};
...
export async function notifySlack(endpoint: string, status: number, errorMsg: string, userId = '') {
  ...
  // 항상 콘솔에 로깅
  console.error(
    `[Error] ${endpoint} ${status}: ${errorMsg} (user: ${userId.substring(0, 8) || 'unknown'})`
  );
  ...
  if (status >= 500) {
    ...
    if (!_errorBuckets[cooldownKey] || now500 - _errorBuckets[cooldownKey].lastNotified > 60000) {
      if (!_errorBuckets[cooldownKey])
        _errorBuckets[cooldownKey] = { count: 0, firstAt: now500, lastNotified: 0, samples: [] };
