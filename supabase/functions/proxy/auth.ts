...
export async function handleSignup(cors: Record<string, string>, req: Request, svc: any) {
  ...
  if (error) {
    if (error.message.includes("already")) return json(cors, { error: "이미 가입된 이메일입니다" }, 409);
  ...
  if (profileErr) {
    try { await svc.auth.admin.deleteUser(data.user.id); } catch (_) { /* best-effort rollback */ }
    await notifySlack("signup", 500, `Profile upsert 실패 → auth user 롤백: ${email} / ${profileErr.message}`, "system");
    return json(cors, { error: "회원가입 처리 중 오류가 발생했습니다. 다시 시도해주세요." }, 500);
  }

  return json(cors, { message: "회원가입 완료. 관리자 승인 후 이용 가능합니다.", status: "대기중" });
}
...
export async function handleLogin(cors: Record<string, string>, req: Request, svc: any) {
  ...
  if (!profile) {
    ...
    const { error: recoverErr } = await svc.from("profiles").upsert({
      id: data.user.id, email, full_name: email.split("@")[0], name: email.split("@")[0],
      phone: "", cohort: "", role: "user", approval_status: "대기중", must_change_password: false,
    });
    if (recoverErr) {
      await notifySlack("login", 500, `고아 유저 profile 복구 실패: ${email} / ${recoverErr.message}`, data.user.id);
