...
Deno.serve(async (req) => {
  ...
  if (path === "/auth/signup" && req.method === "POST") {
    ...
    if ((count || 0) >= 5) return json(cors, { error: "가입 요청이 너무 많습니다. 잠시 후 다시 시도하세요." }, 429);
    const t0 = Date.now();
    const resp = await handleSignup(cors, req, svc);
    await svc.from("usage_logs").insert({ user_id: ip, endpoint: "signup", status_code: resp.status, response_ms: Date.now() - t0 });
    return resp;
  }
  ...
  if (path === "/auth/refresh" && req.method === "POST") {
    ...
    const since = new Date(Date.now() - 300000).toISOString();
    const { count } = await svc.from("usage_logs").select("*", { count: "exact", head: true })
      .eq("endpoint", "refresh").eq("user_id", ip).gte("created_at", since);
    if ((count || 0) >= 30) return json(cors, { error: "토큰 갱신 요청이 너무 많습니다. 5분 후 다시 시도하세요." }, 429);
    const t0 = Date.now();
    const resp = await handleRefresh(cors, req);
    await svc.from("usage_logs").insert({ user_id: ip, endpoint: "refresh", status_code: resp.status, response_ms: Date.now() - t0 });
    return resp;
  }
  ...
  if ((user as any).rejected) return json(cors, { error: "관리자 승인 대기 중입니다", code: "APPROVAL_PENDING" }, 403);
  ...
  try {
    ...
    if (path.startsWith("/api/youtube/")) return handleYouTube(cors, path, url, svc, userId, userRole);
    if (path === "/api/trends") {
      const trendRate = await checkRate(svc, userId, "trends", userRole);
      if (!trendRate.allowed) return json(cors, { error: "요청 한도 초과", code: "RATE_LIMIT" }, 429);
    ...
    if (path === "/api/telemetry" && req.method === "POST") {
      try {
        ...
        if (Array.isArray(events) && events.length > 0 && events.length <= 100) {
          ...
          await svc.from("telemetry_events").insert(rows).catch(() => {});
        }
      } catch (_) {}
